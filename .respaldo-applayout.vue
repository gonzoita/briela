    ...
    <div class="min-h-screen bg-lienzo">
        ...
        >
            ...
            <div class="px-3 py-4 border-t shrink-0" style="border-color: rgba(255,255,255,0.12);">
                <button
                    @click="irPerfil"
                    class="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl transition-colors hover:bg-superficie/10"
                >
                    <div
                ...
                <button
                    @click="cerrarSesion"
                    class="flex items-center gap-3 w-full px-3 py-2 mt-1 rounded-xl text-sm text-blue-200 hover:bg-superficie/10 hover:text-white transition-colors"
                >
                    ...
                    <template v-for="item in navItems" :key="item.href ?? item.label">
                        ...
                        >
                            <span v-if="item.sub" class="w-1.5 h-1.5 rounded-full bg-gray-300 shrink-0 -ml-1"
                                :class="isActive(item.href) ? 'bg-superficie/60' : 'bg-gray-300'"></span>
                            <svg v-if="item.icon === 'home'" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                            </svg>
                            <svg v-if="item.icon === 'clipboard'" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6">

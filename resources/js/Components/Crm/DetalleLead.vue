    ...
    <teleport to="body">
        ...
        <div class="fixed top-0 right-0 bottom-0 z-[56] flex flex-col bg-superficie shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <div v-if="cargando" class="flex-1 flex items-center justify-center">
                <div class="w-8 h-8 border-4 border-linea rounded-full animate-spin" style="border-top-color: var(--marca);"></div>
            </div>

            <template v-else-if="lead">
                <!-- ── A) Header ──────────────────────────────────────────── -->
                <div class="flex items-start justify-between gap-3 px-5 py-4 border-b border-linea shrink-0">
                    <div class="min-w-0">
                        <h2 class="text-base font-semibold text-tinta-900 leading-snug">{{ lead.titulo }}</h2>
                        <div class="flex items-center gap-2 mt-1">
                            <span
                                class="inline-block w-2 h-2 rounded-full shrink-0"
                                :style="{ background: lead.etapa?.color }"
                            ></span>
                            <span class="text-xs text-tinta-400">{{ lead.etapa?.nombre }}</span>
                        </div>
                    </div>
                    <button @click="emit('cerrar')"
                        class="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-tinta-100 transition-colors shrink-0 mt-0.5">
                ...
                <div class="flex-1 overflow-y-auto">
                    ...
                    <div class="px-5 py-4 border-b border-linea">
                        <h3 class="text-xs font-semibold text-tinta-300 uppercase tracking-[0.12em] mb-3">Contacto</h3>
                        <div class="space-y-2">
                            <div v-if="lead.nombre_contacto" class="flex items-center gap-2 text-sm text-tinta-700">
                            ...
                            <div v-if="lead.empresa_contacto" class="flex items-center gap-2 text-sm text-tinta-700">
                                <svg class="w-4 h-4 text-tinta-300 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6">
                            ...
                            <div v-if="lead.fuente" class="flex items-center gap-2 text-sm text-tinta-700">
                                <svg class="w-4 h-4 text-tinta-300 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/>
                                </svg>
                                {{ lead.fuente }}
                            </div>
                            <div v-if="lead.responsable" class="flex items-center gap-2 text-sm text-tinta-700">
                                <svg class="w-4 h-4 text-tinta-300 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                                </svg>
                            ...
                            <div v-if="!lead.cliente" class="mt-3 space-y-2">
                                ...
                                <div v-if="mostrarFormConvertir" class="bg-blue-50 rounded-xl p-3 space-y-2">
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        ...
                                        <div>
                                            <label class="text-xs text-tinta-400 block mb-1">Nombre *</label>
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        <div>
                                            <label class="text-xs text-tinta-400 block mb-1">Email</label>
                                            <input v-model="formConvertir.email" type="email"
                                                class="w-full border border-tinta-200 rounded-lg px-2 py-1.5 text-sm"/>
                                        </div>
                                        <div>
                                            <label class="text-xs text-tinta-400 block mb-1">Celular</label>
                                            <input v-model="formConvertir.celular"
                                                class="w-full border border-tinta-200 rounded-lg px-2 py-1.5 text-sm"/>
                                        </div>
                                    </div>
                                    <div>
                                        <label class="text-xs text-tinta-400 block mb-1">Ciudad</label>
                                    ...
                                    <div class="flex gap-2 pt-1">
                                        ...
                                        <button @click="mostrarFormConvertir = false"
                                            class="text-sm text-tinta-400 px-3 py-1.5">
                                            Cancelar
                                        </button>
                                    </div>
                                </div>
                        ...
                        <div v-if="lead.descripcion" class="mt-3 text-sm text-tinta-500 bg-tinta-50 rounded-xl px-3 py-2.5">
                            {{ lead.descripcion }}
                        </div>
                    </div>
                    ...
                    <div class="px-5 py-4 border-b border-linea">
                        ...
                        <div v-if="mostrarFormTarea" class="bg-blue-50 rounded-xl p-3 mb-3 space-y-2">
                            ...
                            <div class="flex gap-2">
                                <button @click="mostrarFormTarea = false"
                                    class="flex-1 py-2 rounded-lg text-xs font-semibold text-tinta-400 border border-linea hover:bg-tinta-50">
                                    Cancelar
                                </button>
                        ...
                        <div v-if="lead.tareas.length" class="space-y-2">
                            <div v-for="tarea in lead.tareas" :key="tarea.id"
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-tinta-900 leading-snug"
                                        :class="{ 'line-through text-tinta-300': tarea.completada }">
                                        {{ tarea.titulo }}
                                    </p>
                                ...
                                <button @click="eliminarTarea(tarea.id)"
                                    class="text-tinta-200 hover:text-red-400 transition-colors shrink-0 mt-0.5">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
                                    </svg>
                                </button>
                        ...
                        <p v-else class="text-sm text-tinta-300 text-center py-3">Sin tareas</p>
                    </div>

                    <!-- ── D) Actividad ───────────────────────────────── -->
                    <div class="px-5 py-4 border-b border-linea">
                        <h3 class="text-xs font-semibold text-tinta-300 uppercase tracking-[0.12em] mb-3">Actividad</h3>
                        <div v-if="lead.actividades?.length" class="space-y-2">
                            <div v-for="act in lead.actividades" :key="act.id" class="flex items-start gap-2">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs text-tinta-700">{{ act.descripcion }}</p>
                        ...
                        <p v-else class="text-xs text-tinta-300 text-center py-2">Sin actividad registrada</p>
                    </div>

                    <!-- ── F) Notas ─────────────────────────────────────── -->
                    <div class="px-5 py-4 border-b border-linea">
                        <h3 class="text-xs font-semibold text-tinta-300 uppercase tracking-[0.12em] mb-3">Notas</h3>
                        ...
                        <div class="space-y-2">
                            <div v-for="nota in lead.notas" :key="nota.id"
                                class="bg-tinta-50 rounded-xl p-3">
                                <div class="flex items-start justify-between gap-2">
                                    <p class="text-sm text-tinta-700 leading-relaxed flex-1">{{ nota.contenido }}</p>
                                    <button @click="eliminarNota(nota.id)"
                                        class="text-tinta-200 hover:text-red-400 transition-colors shrink-0">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
                                        </svg>
                                    </button>
                        ...
                        <p v-if="!lead.notas.length" class="text-sm text-tinta-300 text-center py-2">Sin notas</p>
                    </div>
                </div>
            ...
            <div class="bg-superficie rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
                ...
                <div class="px-5 py-4 border-t border-linea flex gap-3">
                    <button @click="modalCierre = false"
                        class="flex-1 py-2.5 rounded-xl text-sm font-semibold text-tinta-500 border border-linea">
                        Cancelar
                    </button>

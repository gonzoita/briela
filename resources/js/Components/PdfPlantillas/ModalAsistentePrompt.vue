        ...
        <div class="bg-white w-full sm:max-w-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[95dvh] sm:max-h-[90vh] rounded-t-2xl">
            ...
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm font-bold text-gray-900">Asistente de Prompt IA</p>
                        <p class="text-xs text-gray-400">Paso {{ paso }} de 3</p>
                    </div>
                </div>
                <button @click="emit('cerrar')" class="p-1.5 rounded-lg hover:bg-gray-100 transition-colors text-gray-400 hover:text-gray-600">
            ...
            <div class="px-5 pt-3 pb-1 shrink-0">
                <div class="flex gap-1.5">
                    <div v-for="i in 3" :key="i"
                        ...
                        :class="i <= paso ? 'bg-purple-600' : 'bg-gray-200'"
                    />
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto px-5 py-4">
                ...
                <!-- ══ PASO 1: Diseño ════════════════════════════════════════════ -->
                <div v-if="paso === 1">
                    <h3 class="text-base font-bold text-gray-900 mb-4">¿Cómo quieres que se vea tu plantilla?</h3>

                    <!-- Estilo visual -->
                    <div class="mb-4">
                        <label class="text-xs font-semibold text-gray-500 mb-1.5 block uppercase tracking-wide">Estilo visual</label>
                        <select v-model="diseno.estilo" class="w-full text-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white">
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-4">
                        <div>
                            <label class="text-xs font-semibold text-gray-500 mb-1.5 block uppercase tracking-wide">Color primario</label>
                            <div class="flex items-center gap-2 border border-gray-200 rounded-xl px-3 py-2 bg-white">
                                ...
                                <span class="text-xs font-mono text-gray-700">{{ diseno.colorPrimario }}</span>
                            </div>
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-gray-500 mb-1.5 block uppercase tracking-wide">Color secundario</label>
                            <div class="flex items-center gap-2 border border-gray-200 rounded-xl px-3 py-2 bg-white">
                                <input type="color" v-model="diseno.colorSecundario" class="w-7 h-7 rounded-lg cursor-pointer border-0 p-0 bg-transparent" />
                                <span class="text-xs font-mono text-gray-700">{{ diseno.colorSecundario }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-4">
                        <div>
                            <label class="text-xs font-semibold text-gray-500 mb-1.5 block uppercase tracking-wide">Orientación</label>
                            <div class="flex gap-1.5">
                                <button
                        ...
                        <div>
                            <label class="text-xs font-semibold text-gray-500 mb-1.5 block uppercase tracking-wide">Tamaño papel</label>
                            <select v-model="diseno.papel" class="w-full text-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white">
                    ...
                    <div v-if="diseno.papel === 'personalizado'" class="flex gap-3 mb-4">
                        <div class="flex-1">
                            <label class="text-xs text-gray-500 block mb-1">Ancho (mm)</label>
                            <input v-model="diseno.ancho_mm" type="number" min="50" max="500"
                                class="w-full border border-gray-200 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"/>
                        </div>
                        <div class="flex-1">
                            <label class="text-xs text-gray-500 block mb-1">Alto (mm)</label>
                    ...
                    <div class="mb-4">
                        <div class="flex items-center justify-between mb-2">
                            <label class="text-xs font-semibold text-gray-500 uppercase tracking-wide">¿Incluir logo de la empresa?</label>
                            <button
                        ...
                        <div v-if="diseno.logo" class="mt-2 space-y-3">
                            ...
                            <!-- URL del logo -->
                            <div>
                                <label class="text-xs text-gray-500 block mb-1">URL del logo <span class="text-gray-400">(opcional)</span></label>
                                <input
                                ...
                                />
                                <p class="text-xs text-gray-400 mt-1">En SGI: <code class="bg-gray-100 px-1 rounded">/storage/logos/logo.png</code></p>
                            </div>
                            <!-- Vista previa del logo -->
                            <div v-if="diseno.logo_url" class="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
                                <img
                                    :src="diseno.logo_url"
                            ...
                            <div class="flex items-center gap-2">
                                <label class="text-xs text-gray-500 whitespace-nowrap">Ancho logo (px)</label>
                                <input v-model="diseno.logo_ancho" type="number" min="40" max="300"
                                    class="w-20 border border-gray-200 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"/>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Sección a generar -->
                    <div class="mb-4">
                        <label class="text-xs font-semibold text-gray-500 mb-1.5 block uppercase tracking-wide">¿Para qué sección generar el prompt?</label>
                    ...
                    <!-- Descripción libre -->
                    <div>
                        <label class="text-xs font-semibold text-gray-500 mb-1.5 block uppercase tracking-wide">Describe el diseño con tus palabras <span class="normal-case text-gray-400 font-normal">(opcional)</span></label>
                        <textarea
                            v-model="diseno.descripcionLibre"
                            rows="3"
                            class="w-full text-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none"
                ...
                <div v-else-if="paso === 2">
                    <h3 class="text-base font-bold text-gray-900 mb-1">¿Qué información debe incluir?</h3>
                    <p class="text-xs text-gray-400 mb-4">Selecciona las variables que quieres en tu documento</p>

                    <!-- Grupos regulares -->
                    <div v-for="grupo in gruposRegulares" :key="grupo.grupo" class="mb-5">
                        <div class="flex items-center justify-between mb-1.5">
                            <span class="text-xs font-bold text-gray-500 uppercase tracking-wider">{{ grupo.grupo }}</span>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-0.5">
                            <label
                                ...
                                :key="v.var"
                                class="flex items-start gap-2 p-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors"
                            >
                                <input
                                    type="checkbox"
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-xs text-gray-400 leading-tight mt-0.5">{{ v.desc }}</p>
                                </div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div v-if="grupoTabla.length > 0" class="mb-5">
                        <div class="flex items-center justify-between mb-1.5">
                            <span class="text-xs font-bold text-gray-500 uppercase tracking-wider">Columnas de la tabla</span>
                    ...
                    <div class="bg-gray-50 rounded-xl p-4 space-y-3">
                        <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">Opciones de tabla</p>
                        <div
                ...
                <div v-else-if="paso === 3">
                    <h3 class="text-base font-bold text-gray-900 mb-1">Tu prompt está listo</h3>
                    <p class="text-xs text-gray-400 mb-3">Copia el prompt y pégalo en Gemini o Claude para generar el HTML</p>
                    ...
                    <template v-if="seccionGeneracion === 'todas'">
                        <div class="flex gap-1 mb-3 bg-gray-100 rounded-lg p-0.5">
                            <button v-for="s in [{ key:'header', label:'⬆ Encabezado' }, { key:'body', label:'📄 Cuerpo' }, { key:'footer', label:'⬇ Pie' }]"
                                ...
                                :class="tabPrompt === s.key ? 'bg-white text-purple-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'">
                                {{ s.label }}
                            </button>
                        </div>
                        <textarea
                            ...
                            readonly
                            rows="14"
                            class="w-full text-xs border border-gray-200 rounded-xl p-3 bg-gray-50 resize-none focus:outline-none focus:ring-2 focus:ring-purple-500 leading-relaxed"
                    ...
                    <div class="mt-4 bg-purple-50 border border-purple-100 rounded-xl p-3">
                        <p class="text-xs text-purple-700 font-medium leading-relaxed">
                            <span class="font-bold">1.</span> Copia el prompt →
                            <span class="font-bold">2.</span> Abre Gemini o Claude →
                            <span class="font-bold">3.</span> Pega y genera →
                            <span class="font-bold">4.</span> Copia el HTML →
                            <span class="font-bold">5.</span> Pégalo en el editor (modo Código)
                        </p>
                    </div>
                </div>

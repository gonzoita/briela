...
const form = ref({
    ...
    plantilla_ensamble_id: props.template.plantilla_ensamble_id ?? null,
    activo:                props.template.activo,
    pasos:                 (props.template.pasos ?? []).map(p => ({
        nombre:           p.nombre,
        descripcion:      p.descripcion,
        peso_porcentaje:  parseFloat(p.peso_porcentaje),
        orden:            p.orden,
...
function submit() {
    markClean()
    router.put(`/produccion/templates/${props.template.id}`, {
...
<template>
    <AppLayout :title="`Editar: ${template.nombre}`">

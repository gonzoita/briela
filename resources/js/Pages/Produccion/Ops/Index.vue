...
<template>
    <AppLayout title="Órdenes de Producción">
        ...
        <div class="flex items-center justify-between mb-4">
            <h1 class="text-lg font-bold text-gray-900">Órdenes de Producción</h1>
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
            <button v-for="card in [
                { key: 'borrador',      label: 'Borrador',      color: 'text-gray-600',   border: 'border-gray-200',   ring: 'ring-gray-300'   },
                ...
                <p class="text-2xl font-bold" :class="card.color">{{ metricas[card.key] ?? 0 }}</p>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-4 space-y-3">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <select v-model="filters.estado" class="border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-blue-400">
                    <option v-for="e in ESTADOS" :key="e.value" :value="e.value">{{ e.label }}</option>
                </select>
                <select v-model="filters.responsable_id" class="border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-blue-400">
                    <option value="">Todos los responsables</option>
                    <option v-for="r in responsables" :key="r.id" :value="r.id">{{ r.name }}</option>
                </select>
                <div class="flex gap-2">
                    <input v-model="filters.desde" type="date"
                        class="flex-1 border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-blue-400" title="Desde" />
                    <input v-model="filters.hasta" type="date"
                        class="flex-1 border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-blue-400" title="Hasta" />
                </div>
            </div>
        ...
        <div class="space-y-3">
            <template v-if="ops.data?.length">
                <div v-for="op in ops.data" :key="op.id"
                    class="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow cursor-pointer"
                    @click="router.visit(`/produccion/ops/${op.id}`)">
                    <div class="px-5 py-4 flex items-center gap-4">
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-2 flex-wrap mb-1">
                                <span class="text-sm font-bold text-gray-900">{{ op.numero }}</span>
                            ...
                            <p class="text-xs text-gray-500 truncate">
                                <span v-if="op.cliente_nombre" class="font-medium text-gray-700">{{ op.cliente_nombre }}</span>
                        ...
                        <div class="hidden sm:flex flex-col items-end shrink-0 gap-1">
                            <p class="text-xs text-gray-500">{{ formatFecha(op.fecha_creacion) }}</p>
                            ...
                            <div v-if="saldoPendiente(op) <= 0 && (op.total ?? 0) > 0"
                                ...
                                <span class="text-xs text-gray-400 mt-0.5">Saldo</span>
                            </div>
                    ...
                    <div class="sm:hidden px-5 pb-3 flex items-center justify-between">
                        <span class="text-xs text-gray-400">{{ formatFecha(op.fecha_creacion) }}</span>
            ...
            <div v-else class="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-16 text-center">
                ...
                <p class="text-sm text-gray-500">No hay órdenes de producción.</p>
        ...
        <div v-if="ops.last_page > 1" class="flex items-center justify-between mt-4">
            ...
            <div class="flex gap-2">
                <a v-if="ops.prev_page_url" :href="ops.prev_page_url"
                    class="px-3 py-1.5 rounded-xl border border-gray-200 text-xs text-gray-600 hover:bg-gray-50">Anterior</a>
                <a v-if="ops.next_page_url" :href="ops.next_page_url"
                    class="px-3 py-1.5 rounded-xl border border-gray-200 text-xs text-gray-600 hover:bg-gray-50">Siguiente</a>
            </div>
        </div>

...
<template>
    <AppLayout title="Mi Panel">
        ...
        <div v-if="sin_perfil" class="flex flex-col items-center justify-center min-h-[60vh] gap-4 text-center px-4">
            ...
            <div>
                <h2 class="text-lg font-semibold text-tinta-900">Sin perfil de operario</h2>
                <p class="text-sm text-tinta-400 mt-1">Tu usuario no tiene un perfil de operario asociado.<br>Contacta al administrador.</p>
            </div>
        </div>
        ...
        <template v-else>
            ...
            <div class="bg-white rounded-2xl border border-linea p-5 shadow-sm mb-4">
                <div class="flex items-center gap-4">
                    ...
                    <div class="min-w-0">
                        <h1 class="text-base font-semibold text-tinta-900 truncate">{{ operario?.nombre }}</h1>
                        <p v-if="operario?.especialidad" class="text-sm text-tinta-400 truncate">{{ operario?.especialidad }}</p>
                        <p class="text-xs text-tinta-300 mt-0.5">Doc: {{ operario?.documento }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-linea p-5 shadow-sm mb-4">
                ...
                <div class="flex items-center gap-4 mb-4">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-xs text-tinta-300 uppercase tracking-wide">Nivel actual</p>
                        <p class="text-xl font-semibold" :style="{ color: nivel_actual?.color ?? '#CD7F32' }">
                            {{ nivel_actual?.nombre ?? 'Bronce' }}
                        </p>
                        <p class="text-sm text-tinta-400">{{ puntos_totales }} puntos acumulados</p>
                    </div>
                    <div v-if="posicion_ranking" class="text-center shrink-0">
                        <p class="text-3xl font-black" style="color:var(--marca)">#{{ posicion_ranking }}</p>
                        <p class="text-xs text-tinta-300">esta semana</p>
                ...
                <div v-if="nivel_siguiente">
                    ...
                    <p class="text-xs text-tinta-300 mt-1 text-right">
            ...
            <div v-if="trabajos_hoy.length" class="bg-white rounded-2xl border border-linea p-5 shadow-sm mb-4">
                ...
                <div class="space-y-2">
                    <div v-for="(t, idx) in trabajos_hoy" :key="idx"
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-tinta-900 truncate">{{ t.nombre }}</p>
            ...
            <div v-if="trabajos_activos?.length"
                ...
                <div class="divide-y divide-gray-50">
                    <button
                        ...
                        class="w-full text-left px-4 py-3 hover:bg-tinta-50 transition-colors"
                    >
                        <div class="flex items-start justify-between gap-2 mb-1.5">
                            <div class="min-w-0">
                                ...
                                <p v-if="t.item_descripcion" class="text-xs text-tinta-400 truncate mt-0.5">
                                    {{ t.item_descripcion }}
                                </p>
                            </div>
                            <span class="text-xs font-semibold shrink-0 mt-0.5" style="color:var(--marca);">
                                {{ Math.round(t.porcentaje_avance) }}%
                            </span>
                        </div>
                        <!-- Barra de progreso -->
                        <div class="h-1.5 bg-tinta-100 rounded-full overflow-hidden mb-2">
                        ...
                        <div class="flex gap-1 flex-wrap">
                            <div
                                ...
                                :class="idx <= t.pasos_completados ? 'bg-green-500' : 'bg-tinta-200'"
                            />
                        </div>
                    </button>
                </div>
            ...
            <div v-if="trabajos_terminados?.length"
                ...
                <div class="divide-y divide-gray-50">
                    <button
                        ...
                        :key="t.id"
                        @click="router.visit(`/trabajo/${t.token}`)"
                        class="w-full text-left px-4 py-3 hover:bg-tinta-50 transition-colors opacity-60"
                    >
                        <div class="flex items-center justify-between gap-2">
                            <div class="min-w-0">
                                <p class="text-sm font-medium text-tinta-500 truncate">
                                    {{ t.op_numero }} — U{{ t.numero_unidad }}/{{ t.total_unidades }}
                                </p>
                                <p v-if="t.item_descripcion" class="text-xs text-tinta-300 truncate mt-0.5">
                                    {{ t.item_descripcion }}
                                </p>
                            </div>

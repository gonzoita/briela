    ...
    <AppLayout title="Roles y permisos">
        <div class="max-w-3xl mx-auto">
            ...
            <div class="flex items-center gap-3 mb-5">
                ...
                <h1 class="text-xl font-bold text-gray-900">Roles y permisos</h1>
            </div>
                        ...
                        <div class="flex items-center gap-3">
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 flex-wrap">
                                    <p class="text-sm font-medium text-gray-800 truncate">{{ r.nombre }}</p>
                                    <span v-if="r.es_sistema"
                                        class="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-500">Sistema</span>
                            ...
                            <span class="text-xs px-2 py-0.5 rounded-full shrink-0"
                                :class="r.activo ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-5">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Rol base *</label>
                        <select v-model="form.rol_base" :disabled="esSistema"
                            class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm bg-white disabled:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <div class="flex flex-col gap-2">
                        <div>
                            <label class="flex items-center gap-2 cursor-pointer">
                                ...
                                <span class="text-sm text-gray-700">Acceso a todas las sedes</span>
                            </label>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Permisos</p>
                            <span class="text-xs text-gray-400">{{ totalMarcados }} marcados</span>
                        </div>

                        <div v-for="(modulos, grupo) in catalogo" :key="grupo" class="mb-4">
                            <p class="text-xs font-semibold text-gray-600 mb-2">{{ grupo }}</p>

                            <div v-for="(config, modulo) in modulos" :key="modulo"
                                class="rounded-xl bg-gray-50 p-3 mb-2">
                                <div class="flex items-center justify-between mb-2">
                                ...
                                <div class="flex flex-wrap gap-x-4 gap-y-1.5">
                                    <label v-for="a in config.acciones" :key="a"
                                        ...
                                        <span class="text-sm text-gray-600">{{ etiquetasAcciones[a] ?? a }}</span>
                                    </label>
                                </div>
                            </div>
                        </div>

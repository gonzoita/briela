...
<template>
    <AppLayout title="Configuración del Sistema">
        <div class="max-w-3xl mx-auto">
            ...
            <h1 class="text-xl font-bold text-gray-900 mb-5">Configuración del Sistema</h1>

            <!-- Tabs -->
            <div class="flex gap-1 bg-gray-100 p-1 rounded-2xl mb-5 overflow-x-auto">
                <button
                    ...
                    :class="tab === t ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500'"
                >
            ...
            <div v-show="tab === 'general'" class="space-y-4">
                ...
                >
                    <div class="px-5 py-3 border-b border-gray-100">
                        <h2 class="text-sm font-semibold text-gray-700">{{ grupoLabel[grupo] ?? grupo }}</h2>
                        <p v-if="grupo === 'empresa'" class="text-xs text-gray-500 mt-0.5">
                    ...
                    <div class="p-5 space-y-4">
                        <template v-for="c in items" :key="c.clave">
                            <div v-if="c.clave !== 'empresa_logo_url'">
                                ...
                                />
                                <p v-if="c.descripcion" class="text-xs text-gray-400 mt-1">{{ c.descripcion }}</p>
                            </div>
                        </template>
                        ...
                        <div v-if="grupo === 'empresa'">
                            ...
                            <button
                                ...
                                class="w-full border-2 border-dashed border-gray-300 rounded-xl py-3 text-sm text-gray-500 hover:border-[var(--marca)] hover:text-[var(--marca)] transition-colors"
                            >
                        ...
                        <div v-if="grupo === 'seguridad'" class="mt-4 rounded-xl border border-blue-100 overflow-hidden">
                            ...
                            <div class="bg-white px-4 py-3 space-y-2">
                                <div class="flex items-start gap-2">
                                    <span class="w-5 h-5 rounded-full bg-[var(--marca)] text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5 font-bold">1</span>
                                    <p class="text-xs text-gray-600">Ve a <a href="https://www.google.com/recaptcha/admin" target="_blank" class="text-[var(--marca)] underline font-semibold">google.com/recaptcha/admin</a> e inicia sesión con tu cuenta Google.</p>
                                </div>
                                <div class="flex items-start gap-2">
                                ...
                                <div class="flex items-start gap-2">
                                    ...
                                    <p class="text-xs text-gray-600">Copia la <strong>Clave del sitio (Site Key)</strong> y pégala en el campo <em>reCAPTCHA Site Key</em> de arriba.</p>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="w-5 h-5 rounded-full bg-[var(--marca)] text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5 font-bold">5</span>
                                    <p class="text-xs text-gray-600">Copia la <strong>Clave secreta (Secret Key)</strong> y pégala en el campo <em>reCAPTCHA Secret Key</em> de arriba.</p>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="w-5 h-5 rounded-full bg-[var(--marca)] text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5 font-bold">6</span>
                                    <p class="text-xs text-gray-600">Guarda la configuración y activa reCAPTCHA en cada formulario desde <strong>CRM → Formularios → Editar → Seguridad</strong>.</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div v-show="tab === 'notificaciones'" class="space-y-4">
                ...
                <div v-for="(items, grupo) in notifLocal" :key="grupo"
                    ...
                    <div class="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
                        <h2 class="text-sm font-semibold text-gray-700">{{ grupo }}</h2>
                        <div class="flex items-center gap-6 text-[11px] font-semibold text-gray-400 uppercase tracking-wide">
                            <span>Campanita</span>
                            <span>Email</span>
                        </div>
                    </div>
                    <div class="divide-y divide-gray-50">
                        <div v-for="n in items" :key="n.tipo"
                            ...
                            <span class="text-sm text-gray-800 flex-1">{{ n.label }}</span>
            ...
            <div v-show="tab === 'email'" class="space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <div class="px-5 py-3 border-b border-gray-100">
                        <h2 class="text-sm font-semibold text-gray-700">Correo electrónico (SMTP)</h2>
                        <p class="text-xs text-gray-400 mt-0.5">Credenciales para envío de emails desde el sistema</p>
                    </div>
                    <div class="p-5 space-y-4">
                        <!-- Servidor y Puerto -->
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Servidor SMTP</label>
                                <input
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Puerto</label>
                                <input
                        ...
                        <!-- Cifrado -->
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Cifrado</label>
                            <select
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Usuario (email)</label>
                            <input
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input
                                    ...
                                    placeholder="••••••••"
                                    class="w-full rounded-xl border border-gray-300 px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                                />
                                <button
                                    type="button"
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Nombre remitente</label>
                                <input
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Email remitente</label>
                                <input
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-5">
                    <h3 class="text-sm font-semibold text-gray-700 mb-1">Probar conexión SMTP</h3>
                    <p class="text-xs text-gray-400 mb-3">Guarda primero la configuración y luego envía un email de prueba.</p>
                    <div class="flex gap-2">
                        <input
                            v-model="emailPrueba"
                            type="email"
                            placeholder="destino@ejemplo.com"
                            class="flex-1 rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                        />
                        <button
                            @click="probarSmtp"
            ...
            <div v-show="tab === 'tipos'" class="space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <div class="px-5 py-3 border-b border-gray-100">
                        <h2 class="text-sm font-semibold text-gray-700">Tipos de colaborador</h2>
                        <p class="text-xs text-gray-400 mt-0.5">Arrastra para reordenar</p>
                    </div>
                    <div v-if="!tipos.length" class="py-8 text-center text-sm text-gray-400">Sin tipos configurados.</div>
                    <div class="divide-y divide-gray-50">
                        <div
                        ...
                        >
                            <!-- Handle -->
                            <svg class="w-4 h-4 text-gray-300 cursor-grab shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M4 8h16M4 16h16" />
                            </svg>
                            <!-- Color -->
                            <div class="w-5 h-5 rounded-full shrink-0 border border-gray-200" :style="{ background: t.color }" />
                            <!-- Nombre -->
                            <p class="flex-1 text-sm font-medium text-gray-800 min-w-0 truncate">{{ t.nombre }}</p>
                            <!-- Badge activo -->
                            <span
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-3">
                                <input
                                    ...
                                    type="color"
                                    class="w-10 h-10 rounded-xl border border-gray-300 cursor-pointer p-0.5"
                                />
                                <span class="text-sm text-gray-500 font-mono">{{ tipoForm.color }}</span>
                            </div>
                        </div>
            ...
            <div v-show="tab === 'estaciones'" class="space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <div class="px-5 py-3 border-b border-gray-100">
                        <h2 class="text-sm font-semibold text-gray-700">Estaciones de trabajo</h2>
                        <p class="text-xs text-gray-400 mt-0.5">Arrastra para reordenar</p>
                    </div>
                    <div v-if="!estaciones.length" class="py-8 text-center text-sm text-gray-400">Sin estaciones configuradas.</div>
                    <div class="divide-y divide-gray-50">
                        <div
                        ...
                        >
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="w-5 h-5 rounded-full shrink-0 border border-gray-200" :style="{ background: e.color }" />
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-gray-800 truncate">{{ e.nombre }}</p>
                                    <p v-if="e.descripcion" class="text-xs text-gray-400 truncate">{{ e.descripcion }}</p>
                                </div>
                                <span class="text-xs text-gray-500 shrink-0">Cap: {{ e.capacidad_simultanea }}</span>
                                <span
                                    class="text-xs px-2 py-0.5 rounded-full shrink-0"
                                    :class="e.activa ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'"
                                >
                            ...
                            <div class="mt-2 ml-7">
                                ...
                                <p v-else class="text-xs text-gray-400 italic">Sin equipos asignados</p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-3">
                                <input
                                    ...
                                    class="w-10 h-10 rounded-xl border border-gray-300 cursor-pointer p-0.5"
                                />
                                <span class="text-sm text-gray-500 font-mono">{{ estacionForm.color }}</span>
                            </div>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Capacidad simultánea *</label>
                            <input
            ...
            <div v-show="tab === 'puntos'" class="space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <div class="px-5 py-3 border-b border-gray-100">
                        <h2 class="text-sm font-semibold text-gray-700">Configuración de puntos</h2>
                    </div>
                    <div class="p-5 space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <div class="px-5 py-3 border-b border-gray-100">
                        <h2 class="text-sm font-semibold text-gray-700">Niveles de colaborador</h2>
                        <p class="text-xs text-gray-400 mt-0.5">Define los niveles que se asignan según los puntos acumulados.</p>
                    </div>
                    <div v-if="!niveles.length" class="py-8 text-center text-sm text-gray-400">Sin niveles configurados.</div>
                    <div class="divide-y divide-gray-50">
                        <div v-for="n in niveles" :key="n.id" class="px-4 py-3">
                            <div v-if="editandoNivel === n.id" class="space-y-3">
                                <div class="grid grid-cols-2 gap-2">
                                    ...
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Icono</label>
                                ...
                                <div class="grid grid-cols-3 gap-2">
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Color</label>
                                        <input v-model="nivelForm.color" type="color"
                                            class="w-full h-9 rounded-xl border border-gray-300 px-1 py-1 cursor-pointer" />
                                    </div>
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Pts min</label>
                                        <input v-model.number="nivelForm.puntos_minimos" type="number" min="0"
                                            class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2" />
                                    </div>
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Pts max</label>
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <button @click="cancelarNivel"
                                        class="px-4 py-2 rounded-xl border border-gray-300 text-xs text-gray-600">Cancelar</button>
                                </div>
                            </div>
                            <div v-else class="flex items-center gap-3">
                                <span class="text-2xl leading-none">{{ n.icono }}</span>
                                <div class="w-4 h-4 rounded-full shrink-0 border border-gray-200" :style="{ background: n.color }" />
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-gray-800">{{ n.nombre }}</p>
                                    <p class="text-xs text-gray-400">
                                        {{ n.puntos_minimos }} – {{ n.puntos_maximos ?? '∞' }} pts
                                    </p>
                                </div>
                                <button @click="editarNivel(n)"
                                    class="px-3 py-1 rounded-xl border border-gray-300 text-xs text-gray-600 hover:bg-gray-50">
                                    Editar
                                </button>
                ...
                <!-- Nuevo nivel -->
                <div v-if="!editandoNivel" class="bg-white rounded-2xl border border-gray-200 p-5">
                    <h3 class="text-sm font-semibold text-gray-700 mb-4">Nuevo nivel</h3>
                    <div class="space-y-3">
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Nombre *</label>
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Icono (emoji)</label>
                                <input v-model="nivelForm.icono" type="text" maxlength="4"
                                    class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2" />
                            </div>
                        </div>
            ...
            <div v-show="tab === 'crm'" class="space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <div class="px-5 py-3 border-b border-gray-100">
                        <h2 class="text-sm font-semibold text-gray-700">Etapas del Pipeline</h2>
                        <p class="text-xs text-gray-400 mt-0.5">Arrastra para reordenar</p>
                    </div>
                    <div v-if="!crmEtapas.length" class="py-8 text-center text-sm text-gray-400">Sin etapas configuradas.</div>
                    <div class="divide-y divide-gray-50">
                        <div
                        ...
                        >
                            ...
                            <div class="w-4 h-4 rounded-full shrink-0 border border-gray-200" :style="{ background: etapa.color }"/>
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-medium text-gray-800 truncate">{{ etapa.nombre }}</p>
                                <p class="text-xs text-gray-400">{{ crmAccionLabel[etapa.accion_automatica] }}</p>
                            </div>
                            ...
                            <span class="text-xs px-2 py-0.5 rounded-full shrink-0"
                                :class="etapa.activa ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'">
                    ...
                    <div class="space-y-3">
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    <input v-model="crmEtapaForm.color" type="color"
                                        class="w-10 h-10 rounded-xl border border-gray-300 cursor-pointer p-0.5"/>
                                    <span class="text-sm text-gray-500 font-mono">{{ crmEtapaForm.color }}</span>
                                </div>
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Acción automática</label>
                        ...
                        <div class="flex gap-4">
                            <label class="flex items-center gap-2 cursor-pointer">
                                ...
                                <span class="text-sm text-gray-700">Es etapa "Ganado"</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input v-model="crmEtapaForm.es_perdido" type="checkbox" class="rounded"/>
                                <span class="text-sm text-gray-700">Es etapa "Perdido"</span>
                            </label>
                        </div>
            ...
            <div v-show="tab === 'sistema'" class="space-y-6">
                ...
                <!-- ORGANIZACIÓN -->
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Organización</p>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
                        <a href="/configuracion/sedes" @click.prevent="router.visit('/configuracion/sedes')"
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-gray-800">Sedes</p>
                                <p class="text-xs text-gray-500 mt-0.5">Sedes de ventas y fábricas de la empresa</p>
                            </div>
                        ...
                        <a href="/configuracion/perfil-marca" @click.prevent="router.visit('/configuracion/perfil-marca')"
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-gray-800">Perfil de marca y asistente</p>
                                <p class="text-xs text-gray-500 mt-0.5">Identidad, tono y voz, y el nombre del asistente</p>
                            </div>
                            <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                            </svg>
                        ...
                        <a href="/configuracion/roles" @click.prevent="router.visit('/configuracion/roles')"
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-gray-800">Roles y permisos</p>
                                <p class="text-xs text-gray-500 mt-0.5">Crear roles y definir qué puede hacer cada uno</p>
                            </div>
                            <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                            </svg>
                        ...
                        <a href="/configuracion/numeracion" @click.prevent="router.visit('/configuracion/numeracion')"
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-gray-800">Numeración</p>
                                <p class="text-xs text-gray-500 mt-0.5">Prefijos y consecutivos de cada documento, por sede</p>
                            </div>
                            <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                            </svg>
                        ...
                        <a href="/configuracion/marca" @click.prevent="router.visit('/configuracion/marca')"
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-gray-800">Marca</p>
                                <p class="text-xs text-gray-500 mt-0.5">Color, favicon y título de la pestaña del navegador</p>
                            </div>
                            <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                            </svg>
                        ...
                        <a href="/configuracion/identificacion" @click.prevent="router.visit('/configuracion/identificacion')"
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-gray-800">Identificación de clientes</p>
                                <p class="text-xs text-gray-500 mt-0.5">Dígito de verificación del NIT, duplicados y consulta al RUES</p>
                            </div>
                            <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                            </svg>
                ...
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Inventario y Cotización</p>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <a href="/configuracion/bodegas" @click.prevent="router.visit('/configuracion/bodegas')"
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-gray-800">Bodegas</p>
                                <p class="text-xs text-gray-500 mt-0.5">Crear y gestionar bodegas de almacenamiento</p>
                            </div>
                            <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                            </svg>
                        ...
                        <a href="/cotizadores/plantillas" @click.prevent="router.visit('/cotizadores/plantillas')"
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-gray-800">Plantillas de Ensamble</p>
                                <p class="text-xs text-gray-500 mt-0.5">Constructor de plantillas con fórmulas y componentes</p>
                            </div>
                            <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                            </svg>
                ...
                <!-- RRHH -->
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">RRHH</p>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <a href="/rrhh/configuracion" @click.prevent="router.visit('/rrhh/configuracion')"
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-gray-800">Configuración RRHH</p>
                                <p class="text-xs text-gray-500 mt-0.5">Turnos, tarifas y configuración de nómina</p>
                            </div>
                            <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                            </svg>
                ...
                <!-- ADMINISTRACIÓN -->
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Administración</p>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <a href="/usuarios" @click.prevent="router.visit('/usuarios')"
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-500 mt-0.5">Gestión de usuarios y roles del sistema</p>
                            </div>
                            <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                            </svg>
                        ...
                        <a href="/configuracion/whatsapp-numeros" @click.prevent="router.visit('/configuracion/whatsapp-numeros')"
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-gray-800">Números de WhatsApp</p>
                                <p class="text-xs text-gray-500 mt-0.5">Gestión de números y modo Coexistencia (WhatsApp Cloud API)</p>
                            </div>
                            <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                            </svg>
                        ...
                        <a href="/administracion/backup" @click.prevent="router.visit('/administracion/backup')"
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-gray-800">Backup Base de Datos</p>
                                <p class="text-xs text-gray-500 mt-0.5">Respaldos y restauración de la base de datos</p>
                            </div>
                            <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                            </svg>
                ...
                <!-- PANTALLA DE PLANTA -->
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Pantalla de Planta</p>
                    <div class="bg-white rounded-xl border border-gray-200 p-4">
                        <p class="text-sm font-semibold text-gray-700 mb-1">📺 Pantalla de Planta</p>
                        ...
                        <div class="flex gap-2">
                            ...
                            <a
                                :href="urlPantalla"
                                target="_blank"
                                class="px-3 py-2 rounded-lg text-xs font-medium border border-gray-300 text-gray-600 hover:bg-gray-50 shrink-0"
                            >
                                Ver
                            </a>
                        </div>
                ...
                <!-- CLIENTES -->
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Clientes</p>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <a href="/administracion/segmentacion" @click.prevent="router.visit('/administracion/segmentacion')"
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-gray-800">Segmentación</p>
                                <p class="text-xs text-gray-500 mt-0.5">Tipos de contacto, industrias y procesos de seguimiento</p>
                            </div>
                            <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                            </svg>

    ...
    <AppLayout :title="`Plantilla PDF — ${moduloLabel}`">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex items-center gap-2 text-sm text-gray-500 mb-4">
                <button @click="router.visit('/configuracion/plantillas-pdf')" class="hover:text-gray-700 transition-colors">
                    Plantillas PDF
                </button>
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/>
                </svg>
                <span class="text-gray-900 font-medium">{{ moduloLabel }}</span>
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/>
                </svg>
                <span class="text-gray-600 truncate max-w-xs">{{ form.nombre }}</span>
            </div>

            <!-- Barra de configuración -->
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3 mb-4">
                <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                    ...
                    <input
                        ...
                        class="flex-1 min-w-0 text-sm font-semibold border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[var(--marca)]"
                    />

                    <!-- Papel + Orientación -->
                    <div class="flex flex-wrap gap-2 shrink-0">
                        <select v-model="form.papel" @change="marcarCambio"
                            class="text-xs border border-gray-200 rounded-xl px-2 py-2 focus:outline-none focus:ring-2 focus:ring-[var(--marca)]">
                        ...
                        <template v-if="form.papel === 'personalizado'">
                            <div class="flex items-center gap-1">
                                <span class="text-xs text-gray-400">Ancho</span>
                                <input v-model="form.ancho_mm" @input="marcarCambio"
                                    type="number" min="50" max="500"
                                    class="w-16 text-xs border border-gray-200 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-[var(--marca)]"/>
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        ...
                        <span class="text-xs text-gray-600 whitespace-nowrap">Por defecto</span>
                    </div>

                    <!-- Estado + Guardar -->
                    <div class="flex items-center gap-2 shrink-0">
                        <span class="text-xs hidden sm:inline" :class="guardado ? 'text-green-600' : sinGuardar ? 'text-amber-500' : 'text-gray-400'">
            ...
            <div class="flex lg:hidden mb-3 bg-white rounded-xl border border-gray-100 p-1 shadow-sm">
                <button
                    ...
                    :class="tabActivo === t.key ? 'bg-[var(--marca)] text-white' : 'text-gray-500 hover:text-gray-700'"
                >{{ t.label }}</button>
            </div>
            ...
            <div class="flex flex-col lg:flex-row gap-4" style="min-height: 65vh;">
                ...
                >
                    <!-- Toggle modo editor -->
                    <div class="bg-white rounded-xl border border-gray-100 shadow-sm px-3 py-2 flex flex-wrap items-center gap-2">
                        ...
                        <div class="flex items-center gap-1 bg-gray-100 rounded-lg p-0.5">
                            <button
                                ...
                                :class="modoEditor === 'visual' ? 'bg-white text-[var(--marca)] shadow-sm' : 'text-gray-500 hover:text-gray-700'"
                            >
                                🎨 Visual
                            </button>
                            <button
                                ...
                                class="px-3 py-1.5 text-xs rounded-md font-medium transition-all"
                                :class="modoEditor === 'codigo' ? 'bg-white text-[var(--marca)] shadow-sm' : 'text-gray-500 hover:text-gray-700'"
                            >
                                &lt;/&gt; Código
                            </button>
                        </div>
                        ...
                        <template v-if="modoEditor === 'codigo'">
                            ...
                            <div class="relative">
                                <button
                                    ...
                                    class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors"
                                >
                                    <svg class="w-3.5 h-3.5 text-[var(--marca)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z"/>
                                    </svg>
                                    Insertar variable
                                </button>
                                <div v-if="dropdownVars"
                                    class="absolute top-full left-0 mt-1 z-20 bg-white border border-gray-200 rounded-xl shadow-lg py-1 w-80 max-h-80 overflow-y-auto"
                                    @click.stop>
                                    <template v-for="grupo in variablesAgrupadas" :key="grupo.grupo">
                                        <div class="px-3 pt-2 pb-0.5 sticky top-0 bg-white border-b border-gray-50">
                                            <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">{{ grupo.grupo }}</span>
                                        </div>
                                        <div v-for="v in grupo.vars" :key="v.var"
                                            ...
                                            <p class="text-xs text-gray-400 mt-0.5">{{ v.desc }}</p>
                                        </div>
                                    </template>
                                </div>
                            </div>
                        ...
                        <template v-if="modoEditor === 'codigo'">
                            ...
                            <button
                                ...
                                :class="mostrarVars ? 'bg-blue-50 border-[var(--marca)] text-[var(--marca)]' : 'border-gray-200 text-gray-700 hover:bg-gray-50'"
                            >
                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <div v-if="modoEditor === 'visual'"
                        class="flex bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                        <button
                        ...
                        <template v-else>
                            <div class="flex-1 min-w-0 flex flex-col">
                                <textarea
                                    ...
                                    class="flex-1 w-full rounded-xl border border-gray-200 shadow-sm p-3 text-xs leading-relaxed resize-none focus:outline-none focus:ring-2 focus:ring-[var(--marca)] bg-gray-50"
                            ...
                            <div v-if="mostrarVars"
                                class="w-52 shrink-0 bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col overflow-hidden">
                                <div class="px-3 py-2 border-b border-gray-100">
                                    <p class="text-xs font-semibold text-gray-700">Variables disponibles</p>
                                </div>
                                <div class="flex-1 overflow-y-auto">
                                    <template v-for="grupo in variablesAgrupadas" :key="grupo.grupo">
                                        ...
                                        <div class="p-2 space-y-0.5">
                                            <div v-for="v in grupo.vars" :key="v.var"
                                                class="rounded-lg p-2 hover:bg-gray-50 transition-colors group">
                                                <div class="flex items-start justify-between gap-1">
                                                    ...
                                                    <button
                                                        ...
                                                        :class="copiadoVar === v.var ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'">
                                                        {{ copiadoVar === v.var ? '✓' : 'Copiar' }}
                                                    </button>
                                                </div>
                                                <p class="text-xs text-gray-400 mt-0.5 leading-tight">{{ v.desc }}</p>
                                            </div>
                                        </div>
                                    </template>
                                </div>
                ...
                >
                    <div class="bg-white rounded-xl border border-gray-100 shadow-sm px-3 py-2 flex items-center justify-between">
                        <span class="text-xs font-medium text-gray-700">Vista previa</span>
                        <div class="flex items-center gap-2">
                            <button
                                ...
                                style="background-color: var(--marca);"
                            >
                                <svg class="w-3.5 h-3.5" :class="cargandoPreview ? 'animate-spin' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <div class="flex-1 bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col" style="min-height: 480px;">
                        ...
                        <div v-else class="flex-1 flex flex-col items-center justify-center text-center p-8">
                            ...
                            <p class="text-sm text-gray-400 mb-1">Sin vista previa</p>
                            <p class="text-xs text-gray-300">Haz clic en "Actualizar" para generar el PDF</p>
                        </div>
                    </div>
                </div>
            </div>

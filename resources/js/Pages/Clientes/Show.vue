        ...
        <div class="max-w-3xl mx-auto px-4 py-4">
            ...
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-3">
                    ...
                    <h1 class="text-xl font-bold text-gray-900">Detalle del cliente</h1>
                </div>
                <div class="flex gap-2">
                    <a :href="`/clientes/${cliente.id}/edit`"
                       class="px-3 py-1.5 rounded-lg text-xs font-medium border border-gray-300 text-gray-700 hover:bg-gray-50">
                        Editar
                    </a>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-5 mb-4">
                <div class="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <p v-if="cliente.numero_identificacion" class="text-sm text-gray-500 mt-0.5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div v-if="cliente.celular">
                        <p class="text-xs font-medium text-gray-500 mb-0.5">Celular</p>
                        <a :href="`tel:${cliente.celular}`" class="text-sm text-gray-900 hover:text-blue-600">{{ cliente.celular }}</a>
                    </div>
                    <div v-if="cliente.telefono">
                        <p class="text-xs font-medium text-gray-500 mb-0.5">Teléfono fijo</p>
                        <p class="text-sm text-gray-900">{{ cliente.telefono }}</p>
                    </div>
                    <div v-if="cliente.ciudad">
                        <p class="text-xs font-medium text-gray-500 mb-0.5">Ciudad</p>
                        <p class="text-sm text-gray-900">{{ cliente.ciudad }}</p>
                    </div>
                    <div v-if="cliente.direccion" class="sm:col-span-2">
                        <p class="text-xs font-medium text-gray-500 mb-0.5">Dirección</p>
            ...
            <!-- Notas -->
            <div v-if="cliente.notas" class="bg-white rounded-xl border border-gray-200 p-4 mb-4">
                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Notas</p>
                <p class="text-sm text-gray-700 whitespace-pre-line">{{ cliente.notas }}</p>
            </div>
                ...
                <div v-if="contactos.length > 0" class="space-y-3">
                    <div v-for="c in contactos" :key="c.id"
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p v-if="c.cargo" class="text-xs text-gray-500 mt-0.5">{{ c.cargo }}</p>
                            <div class="flex flex-wrap gap-x-4 gap-y-1 mt-1">
                                <a v-if="c.email" :href="`mailto:${c.email}`" class="text-xs text-blue-600 hover:underline">{{ c.email }}</a>
                                <a v-if="c.celular" :href="`tel:${c.celular}`" class="text-xs text-gray-600">{{ c.celular }}</a>
                                <span v-if="c.telefono" class="text-xs text-gray-600">{{ c.telefono }}</span>
                            </div>
                            <p v-if="c.notas" class="text-xs text-gray-400 mt-1 italic">{{ c.notas }}</p>
                        </div>
                    </div>
                </div>
                <p v-else class="text-sm text-gray-400 text-center py-6">Sin contactos registrados.</p>
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4 mb-4">
                ...
                <div v-if="archivos.length > 0" class="space-y-2">
                    <a v-for="a in archivos" :key="a.id"
                       ...
                       class="flex items-center gap-3 p-2.5 rounded-lg border border-gray-100 hover:bg-gray-50 transition-colors">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-900 truncate">{{ a.nombre_original }}</p>
                ...
                <p v-else class="text-sm text-gray-400 text-center py-6">Sin documentos adjuntos.</p>
            </div>
            ...
            <div v-for="b in bloques" :key="b.clave"
                ...
                <div class="flex items-center justify-between mb-3">
                    <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">
                        {{ b.etiqueta }}
                        <span v-if="b.items.length" class="ml-1 text-gray-300">{{ b.items.length }}</span>
                    </p>
                ...
                <div v-if="b.items.length" class="space-y-1.5">
                    <a v-for="it in b.items" :key="it.id" :href="it.url"
                       class="flex items-center gap-3 p-2.5 rounded-lg border border-gray-100 hover:bg-gray-50 transition-colors">
                        ...
                        <span v-if="it.total" class="text-sm font-semibold text-gray-700 shrink-0">
                            {{ dinero(it.total) }}
                        </span>
                ...
                <p v-else class="text-sm text-gray-400 text-center py-5">{{ b.vacio }}</p>
            </div>

...
const ic = (extra = '') =>
    `w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none ${extra}`
</script>
    ...
    <AppLayout title="Insumos y Precios">
        <div class="max-w-5xl mx-auto">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center gap-3 mb-5">
                <div class="flex-1">
                    <h1 class="text-xl font-bold text-gray-900">Insumos y Precios</h1>
                    <p class="text-sm text-gray-500 mt-0.5">Tabla de precios base usada por el calculador de puertas</p>
                </div>
                <div class="flex gap-2">
                    ...
                    <div class="relative flex-1 sm:flex-none">
                        <input
                            ...
                            class="w-full sm:w-52 rounded-xl border border-gray-300 pl-9 pr-3 py-2 text-sm focus:ring-2 focus:outline-none"
                ...
                <div v-for="(grupo, idx) in agrupados" :key="grupo.categoria"
                    ...
                    >
                        <div class="flex items-center gap-2">
                            <span class="text-sm font-semibold text-gray-800">{{ grupo.categoria }}</span>
                            ...
                            <table class="w-full text-sm">
                                <thead>
                                    <tr style="background:#F8FAFC; border-top: 1px solid #E5E7EB;">
                                        <th class="text-left text-xs font-semibold text-gray-500 uppercase px-4 py-2">Nombre</th>
                                        <th class="text-center text-xs font-semibold text-gray-500 uppercase px-3 py-2 w-16">Unidad</th>
                                        <th class="text-right text-xs font-semibold text-gray-500 uppercase px-4 py-2 w-36">Precio Costo</th>
                                        <th v-if="esAdmin" class="text-center text-xs font-semibold text-gray-500 uppercase px-3 py-2 w-24">Acciones</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-50">
                                    <tr v-for="item in grupo.items" :key="item.id"
                                        :class="['transition-colors', item.activo ? 'hover:bg-gray-50' : 'opacity-50']">
                                        <td class="px-4 py-2.5 font-medium text-gray-800">
                                            {{ item.nombre }}
                                            <span v-if="!item.activo" class="ml-2 text-xs text-red-400">(inactivo)</span>
                                        </td>
                                        <td class="px-3 py-2.5 text-center">
                                            <span class="text-xs font-mono text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded">{{ item.unidad }}</span>
                                        </td>
                                        <td class="px-4 py-2.5 text-right">
                                            ...
                                            <div v-if="editandoId === item.id" class="flex items-center gap-2 justify-end">
                                                ...
                                                <button @click="cancelarEdicion"
                                                    class="w-6 h-6 rounded flex items-center justify-center text-gray-400 hover:bg-gray-100">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                            ...
                                            <div v-else class="flex items-center gap-2 justify-end">
                                                ...
                                                <button v-if="esAdmin" @click="iniciarEdicion(item)"
                                                    class="w-5 h-5 rounded flex items-center justify-center text-gray-300 hover:text-blue-500"
                                                    title="Editar precio">
        ...
        <Teleport to="body">
            <div v-if="modalAgregar" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" style="background:rgba(0,0,0,0.5);">
                <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-5">
                    <h3 class="text-base font-semibold text-gray-900 mb-4">Agregar insumo</h3>
                    <div class="space-y-3">
                        <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Precio costo *</label>

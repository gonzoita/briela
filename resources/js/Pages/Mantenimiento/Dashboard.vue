    ...
    <AppLayout title="Mantenimiento">
        <div class="max-w-4xl mx-auto space-y-5">
            ...
            <div class="flex items-center justify-between">
                <h1 class="text-xl font-bold text-gray-900">Mantenimiento</h1>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div class="bg-white rounded-2xl border border-gray-200 p-4 text-center">
                    <p class="text-2xl font-bold text-gray-900">{{ stats.total_equipos }}</p>
                    <p class="text-xs text-gray-500 mt-0.5">Equipos</p>
                </div>
                ...
                <div class="bg-amber-50 rounded-2xl border border-amber-200 p-4 text-center">
                    <p class="text-2xl font-bold text-amber-700">{{ stats.en_mantenimiento }}</p>
                    <p class="text-xs text-amber-600 mt-0.5">En mant.</p>
                </div>
                <div class="bg-blue-50 rounded-2xl border border-blue-200 p-4 text-center">
                    <p class="text-2xl font-bold text-blue-700">{{ stats.completados_mes }}</p>
            ...
            <div v-if="alertas.length" class="bg-red-50 rounded-2xl border border-red-200 p-4">
                ...
                <div class="space-y-2">
                    <div v-for="e in alertas" :key="e.id"
                        ...
                        <div>
                            <p class="text-sm font-medium text-gray-800">{{ e.nombre }}</p>
            ...
            <div v-if="proximos.length" class="bg-amber-50 rounded-2xl border border-amber-200 p-4">
                <p class="text-sm font-bold text-amber-700 mb-3">Revisiones en los próximos 7 días</p>
                <div class="space-y-2">
            ...
            <!-- En proceso -->
            <div v-if="en_proceso.length" class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                <div class="px-5 py-3 border-b border-gray-100">
                    <h2 class="text-sm font-semibold text-gray-700">En proceso ahora</h2>
                </div>
                <div class="divide-y divide-gray-50">
                    <div v-for="m in en_proceso" :key="m.id"
                        class="flex items-center px-5 py-3 gap-3 hover:bg-gray-50 cursor-pointer"
                        @click="router.visit(`/mantenimiento/mantenimientos/${m.id}`)">
                        <span class="w-2 h-2 rounded-full bg-blue-400 shrink-0"></span>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-800">{{ m.equipo?.nombre }}</p>
                            <p class="text-xs text-gray-400">{{ m.tipo }} · {{ m.ejecutor_nombre ?? 'Sin ejecutor' }}</p>
                        </div>
            ...
            <div v-if="!alertas.length && !proximos.length && !en_proceso.length"
                ...
                <p class="text-sm font-medium text-gray-600">Todo al día</p>
                <p class="text-xs text-gray-400 mt-1">No hay alertas ni revisiones pendientes esta semana</p>
            </div>

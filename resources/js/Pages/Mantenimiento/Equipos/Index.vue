    ...
    <AppLayout title="Equipos">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="flex items-center justify-between mb-5">
                <h1 class="text-xl font-semibold text-tinta-900">Equipos</h1>
            ...
            <div class="flex flex-wrap gap-2 mb-4">
                <input v-model="search" @input="buscar" type="text" placeholder="Buscar equipo..."
                    class="flex-1 min-w-[140px] rounded-xl border border-linea px-3 py-2 text-sm focus:outline-none focus:ring-2" />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div v-for="e in equipos" :key="e.id"
                    ...
                    <div class="flex items-start justify-between gap-3 mb-3">
                        <div class="min-w-0">
                            <p class="font-semibold text-tinta-900 truncate">{{ e.nombre }}</p>
                            <p class="text-xs text-tinta-300 mt-0.5">{{ e.codigo ?? '—' }} · {{ e.tipo ?? 'Tipo no definido' }}</p>
                        </div>
                        <span class="text-xs px-2 py-0.5 rounded-full font-medium shrink-0"
                            :class="estadoBadge[e.estado]?.class ?? 'bg-tinta-100 text-tinta-400'">
                    ...
                    <div class="text-xs text-tinta-400 space-y-1">
                        ...
                        <div v-if="e.ubicacion" class="flex items-center gap-1.5">
                            <svg class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" stroke-width="1.6" viewBox="0 0 24 24">
                        ...
                        <p class="text-tinta-300">{{ e.mantenimientos_count }} mantenimiento(s)</p>
                    </div>
                    <div class="flex gap-2 mt-3 pt-3 border-t border-linea">
                        <button @click.stop="router.visit(`/mantenimiento/equipos/${e.id}/editar`)"
                            class="flex-1 py-1.5 rounded-lg border border-linea text-xs font-medium text-tinta-500 hover:bg-tinta-50">
                            Editar
                        </button>

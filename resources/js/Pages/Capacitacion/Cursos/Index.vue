    ...
    <AppLayout title="Capacitación">
        <div class="max-w-5xl mx-auto">
            ...
            <div class="flex items-center justify-between mb-5 flex-wrap gap-3">
                <div>
                    <h1 class="text-xl font-bold text-gray-900">Capacitación</h1>
                    <p class="text-sm text-gray-500 mt-0.5">Cursos virtuales para colaboradores, contratistas y clientes</p>
                </div>
            ...
            <!-- Vacío -->
            <div v-if="cursos.length === 0" class="bg-white rounded-2xl shadow-sm py-16 text-center text-gray-400">
            ...
            <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                <div v-for="c in cursos" :key="c.id"
                    ...
                    <div class="aspect-video overflow-hidden" style="background:#F1F5F9;">
                        ...
                        <div v-else class="w-full h-full flex items-center justify-center text-white text-3xl font-bold" style="background:var(--marca);">
                            {{ inicial(c.titulo) }}
                        </div>
                    </div>
                    <div class="p-3">
                        <p class="text-sm font-semibold text-gray-900 line-clamp-2 mb-1">{{ c.titulo }}</p>
                        <p v-if="c.categoria" class="text-xs text-gray-400 mb-2">{{ c.categoria }}</p>
                        ...
                        <p class="text-xs text-gray-500 mb-2">{{ c.modulos_count }} módulo{{ c.modulos_count === 1 ? '' : 's' }} · {{ c.lecciones_count }} lección{{ c.lecciones_count === 1 ? '' : 'es' }}</p>
                        <div class="flex items-center justify-between" @click.stop>
                            ...
                            <button @click="eliminar(c)" class="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors">

    ...
    <AppLayout :title="p.nombre">
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-5">
            ...
            <div class="md:col-span-2 space-y-4">
                <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <div class="flex items-center justify-center" style="height:288px; background:#F1F5F9;">
                        ...
                        />
                        <span v-else
                            class="text-5xl font-bold text-white rounded-full flex items-center justify-center"
                ...
                <div v-if="p.descripcion_larga" class="hidden md:block bg-white rounded-2xl shadow-sm p-5">
                    ...
                    <div class="tiptap-content text-sm text-gray-600 leading-relaxed" v-html="p.descripcion_larga"></div>
                </div>
                ...
                <div v-if="p.tipo === 'ensamble'" class="bg-white rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <div class="p-4">
                        <div v-if="!p.componentes?.length" class="text-center text-gray-400 text-sm py-6">
                            Sin componentes.
                            ...
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="border-b border-gray-100">
                                        <th class="text-left text-xs text-gray-500 pb-2 font-medium">#</th>
                                        <th class="text-left text-xs text-gray-500 pb-2 font-medium">Producto</th>
                                        <th class="text-center text-xs text-gray-500 pb-2 font-medium">Cant.</th>
                                        <th class="text-right text-xs text-gray-500 pb-2 font-medium">Subtotal</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="(item, idx) in p.componentes" :key="item.id" class="border-b border-gray-50">
                                        ...
                                        <td class="py-2">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div v-else class="w-7 h-7 rounded-lg flex items-center justify-center text-white text-xs font-bold shrink-0" style="background:var(--marca);">
                                                    {{ item.nombre.charAt(0) }}
                                                </div>
                                                <div class="min-w-0">
                                                    <p class="font-medium text-gray-800 truncate text-xs">{{ item.nombre }}</p>
                                                    <p class="text-xs text-gray-400">×{{ item.cantidad }}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="py-2 text-center text-xs text-gray-700">{{ item.cantidad }}</td>
                                        <td class="py-2 text-right text-xs font-semibold text-gray-800">{{ formatCOP(item.precio_costo_snapshot * item.cantidad) }}</td>
                                    </tr>
                                </tbody>
                                <tfoot>
                                    <tr>
                                        <td colspan="3" class="pt-3 text-xs font-semibold text-gray-600 text-right">Total costo:</td>
                                        <td class="pt-3 text-right font-bold text-sm" style="color:var(--marca);">{{ formatCOP(totalComponentes) }}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                ...
                <div v-if="p.tipo === 'ensamble' && p.configuracion_puerta" class="bg-white rounded-2xl shadow-sm overflow-hidden">
                    <div class="px-5 py-4 border-b border-gray-100">
                        <h3 class="text-sm font-semibold text-gray-700">Configuración de puerta</h3>
                    </div>
                    <div class="p-4">
                        <div class="grid grid-cols-2 gap-3 mb-4 text-sm">
                            <div><p class="text-xs text-gray-400">Tipo</p><p class="font-medium text-gray-800 text-xs">{{ p.configuracion_puerta.tipo_puerta?.replace(/_/g,' ') }}</p></div>
                            <div><p class="text-xs text-gray-400">Dimensiones</p><p class="font-medium text-gray-800 text-xs">{{ p.configuracion_puerta.ancho_vano }}m × {{ p.configuracion_puerta.alto_vano }}m</p></div>
                            <div><p class="text-xs text-gray-400">Temperatura</p><p class="font-medium text-gray-800 text-xs">{{ p.configuracion_puerta.temperatura }}</p></div>
                            <div><p class="text-xs text-gray-400">Lámina</p><p class="font-medium text-gray-800 text-xs">{{ p.configuracion_puerta.lamina }}</p></div>
                        </div>
                        <div class="border border-gray-100 rounded-xl overflow-hidden">
                            <div class="px-3 py-1.5 bg-gray-50 border-b border-gray-100">
                                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Precios por canal</p>
                            </div>
                            <div class="divide-y divide-gray-50">
                                <div class="flex justify-between px-3 py-2"><span class="text-xs text-gray-500">Costo</span><span class="text-xs font-semibold text-gray-800">{{ formatCOP(p.configuracion_puerta.costo) }}</span></div>
                                <div class="flex justify-between px-3 py-2"><span class="text-xs text-gray-500">Mayorista</span><span class="text-xs font-semibold text-gray-800">{{ formatCOP(p.configuracion_puerta.mayorista) }}</span></div>
                                <div class="flex justify-between px-3 py-2"><span class="text-xs text-gray-500">Distribuidor</span><span class="text-xs font-semibold text-gray-800">{{ formatCOP(p.configuracion_puerta.distribuidor) }}</span></div>
                                <div class="flex justify-between px-3 py-2 bg-blue-50"><span class="text-xs font-semibold" style="color:var(--marca);">Cliente Final</span><span class="text-xs font-bold" style="color:var(--marca);">{{ formatCOP(p.configuracion_puerta.cliente_final) }}</span></div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="md:col-span-3 space-y-4">
                ...
                <div class="bg-white rounded-2xl shadow-sm p-5">
                    <!-- Nombre y badges -->
                    <h1 class="text-xl font-bold text-gray-900 mb-2 leading-snug">{{ p.nombre }}</h1>
                    ...
                    <p class="font-mono text-xs text-gray-400 mb-3">{{ p.referencia }} · {{ p.unidad_medida }}</p>
                    <p v-if="p.descripcion_corta" class="text-sm text-gray-600 mb-4">{{ p.descripcion_corta }}</p>

                    <!-- Descripción larga (mobile) -->
                    <div v-if="p.descripcion_larga" class="md:hidden mb-4">
                        <h3 class="text-sm font-semibold text-gray-700 mb-1">Descripción</h3>
                        <div class="tiptap-content text-sm text-gray-600 leading-relaxed" v-html="p.descripcion_larga"></div>
                    </div>
                    ...
                    <div class="border border-gray-100 rounded-xl overflow-hidden mb-4">
                        <div class="px-3 py-2 bg-gray-50 border-b border-gray-100">
                            <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Lista de precios</p>
                        </div>
                        <div class="divide-y divide-gray-50">
                            <div class="flex items-center justify-between px-3 py-2.5">
                                <span class="text-xs text-gray-500">Precio Costo</span>
                                <span class="text-sm font-semibold text-gray-800">{{ formatCOP(p.precio_costo) }}</span>
                            </div>
                            <div class="flex items-center justify-between px-3 py-2.5">
                                <span class="text-xs text-gray-500">Mayorista</span>
                                <span class="text-sm font-semibold text-gray-800">{{ formatCOP(p.precio_mayorista) }}</span>
                            </div>
                            <div class="flex items-center justify-between px-3 py-2.5">
                                <span class="text-xs text-gray-500">Distribuidor</span>
                                <span class="text-sm font-semibold text-gray-800">{{ formatCOP(p.precio_distribuidor) }}</span>
                            </div>
                            <div v-if="p.tipo !== 'ensamble'" class="flex items-center justify-between px-3 py-2.5 bg-blue-50">
                                ...
                                <span class="text-sm font-bold" style="color:var(--marca);">{{ formatCOP(p.precio_cliente_final) }}</span>
                            </div>
                        </div>
                    </div>

                    <!-- Variantes (solo padre) -->
                    <div v-if="p.es_padre" class="border border-gray-100 rounded-xl overflow-hidden mb-4">
                        <div class="px-3 py-2 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                            <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Variantes ({{ p.atributo_variante }})</p>
                        ...
                        <div class="divide-y divide-gray-50">
                            ...
                            <p v-if="!p.variantes?.length" class="text-xs text-gray-400 text-center py-3">Sin variantes.</p>
                        </div>
                    </div>

                    <!-- Inventario por bodega -->
                    <div v-if="p.inventariable && !p.es_padre" class="border border-gray-100 rounded-xl overflow-hidden mb-4">
                        <div class="px-3 py-2 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                        ...
                        <div class="p-3 space-y-3">
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-1">
                                    <span class="text-xs text-gray-500">Stock total</span>
                                    <span class="text-sm font-bold" :style="`color:${stockColor};`">{{ p.stock_total }} {{ p.unidad_medida }}</span>
                                </div>
                                ...
                                <div class="flex justify-between mt-1">
                                    <span class="text-xs text-gray-400">Mín: {{ p.stock_minimo }}</span>
                                    <span class="text-xs text-gray-400">Máx: {{ p.stock_maximo }}</span>
                                </div>
                            </div>
                            <!-- Tabla por bodega -->
                            <div v-if="p.stocks?.length" class="divide-y divide-gray-50">
                                <div v-for="s in p.stocks" :key="s.bodega_id"
                                    ...
                                    <span class="text-xs font-semibold text-gray-800">{{ s.cantidad }}</span>
                                </div>
                            </div>
                            <p v-else class="text-xs text-gray-400 text-center py-1">Sin movimientos registrados</p>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-2 mb-1">
                        <a :href="`/catalogo/productos/${p.id}`" target="_blank"
                            class="flex-1 py-2.5 rounded-xl border border-gray-200 text-xs text-gray-600 font-medium text-center hover:bg-gray-50">
                            Catálogo (con precio)
                        </a>
                    ...
                    <div class="flex gap-2 mb-2">
                        <a :href="`/catalogo/productos/${p.id}?precio=0`" target="_blank"
                            class="flex-1 py-2.5 rounded-xl border border-dashed border-gray-200 text-xs text-gray-400 font-medium text-center hover:bg-gray-50">
                            Catálogo sin precio
                        </a>
        ...
        <div v-if="p.movimientos_recientes?.length" class="mt-5 bg-white rounded-2xl shadow-sm overflow-hidden">
            <div class="px-5 py-3 border-b border-gray-100">
                <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Movimientos recientes</h3>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-xs min-w-[480px]">
                    <thead>
                        <tr style="background:#F8FAFC; border-bottom:1px solid #E5E7EB;">
                            <th class="text-left px-4 py-2.5 font-semibold text-gray-500">Fecha</th>
                            <th class="text-left px-3 py-2.5 font-semibold text-gray-500">Tipo</th>
                            <th class="text-right px-3 py-2.5 font-semibold text-gray-500">Cantidad</th>
                            <th class="text-left px-3 py-2.5 font-semibold text-gray-500">Bodega</th>
                            <th class="text-left px-3 py-2.5 font-semibold text-gray-500">Usuario</th>
                            <th class="text-left px-3 py-2.5 font-semibold text-gray-500">Notas</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50">
                        <tr v-for="mv in p.movimientos_recientes" :key="mv.id" class="hover:bg-gray-50">
                            <td class="px-4 py-2 text-gray-400">{{ fmtFecha(mv.created_at) }}</td>
                            <td class="px-3 py-2">
                            ...
                            <td class="px-3 py-2 text-gray-500">{{ mv.usuario?.name ?? '—' }}</td>
                            <td class="px-3 py-2 text-gray-400 truncate max-w-xs">{{ mv.notas ?? '—' }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <Teleport to="body">
            <div v-if="confirmarEliminar" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" style="background: rgba(0,0,0,0.5);">
                <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm p-5">
                    <h3 class="text-base font-semibold text-gray-900 mb-2">¿Eliminar producto?</h3>
                    <p class="text-sm text-gray-500 mb-5">Esta acción no se puede deshacer. El producto quedará archivado.</p>
                    <div class="flex gap-3">
                        <button @click="confirmarEliminar = false" class="flex-1 py-3 rounded-xl border border-gray-200 text-sm text-gray-600">Cancelar</button>
            ...
            <div v-if="modalAjuste" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" style="background:rgba(0,0,0,0.5);">
                <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-5">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <button @click="modalAjuste = false" class="text-gray-400 hover:text-gray-600 text-xl">✕</button>
                    </div>
                    <div class="space-y-3">
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-1">Bodega</label>
                            <select v-model="formAjuste.bodega_id" class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none">
                        ...
                        <div>
                            ...
                            <select v-model="formAjuste.tipo" class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none">
                        ...
                        <div v-if="formAjuste.tipo === 'transferencia'">
                            <label class="block text-xs font-medium text-gray-600 mb-1">Bodega destino</label>
                            <select v-model="formAjuste.bodega_destino_id" class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none">
                    ...
                    <div class="flex gap-3 mt-5">
                        <button @click="modalAjuste = false" class="flex-1 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-600">Cancelar</button>

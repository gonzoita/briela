...
<template>
    <div class="min-h-screen" style="background:#F8F9FA;">
        ...
        <div class="max-w-xl mx-auto px-4 py-5 space-y-4">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
                ...
                <div class="space-y-1.5 text-sm">
                    <div v-if="op.cliente_nombre" class="flex justify-between">
                        ...
                        <span class="font-medium text-gray-800">{{ op.cliente_nombre }}</span>
                    </div>
                    <div v-if="op.fecha_creacion" class="flex justify-between">
                        <span class="text-gray-500">Creada</span>
                        <span class="text-gray-700">{{ op.fecha_creacion }}</span>
                    </div>
            ...
            <!-- Ítems -->
            <div v-if="op.items?.length" class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
                <div class="px-4 py-3 border-b border-gray-100">
                    <h2 class="text-sm font-semibold text-gray-700">Ítems en producción</h2>
                </div>
                <div class="divide-y divide-gray-50">
                    <div v-for="(item, idx) in op.items" :key="idx" class="px-4 py-3">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm font-medium text-gray-800 flex-1 mr-3">{{ item.descripcion }}</span>
                        ...
                        <div v-if="item.trabajos?.length" class="space-y-1">
                            <div v-for="t in item.trabajos" :key="t.numero_unidad"
                                ...
                                <span class="text-xs text-gray-400 w-16 shrink-0">
                        ...
                        <div v-else class="text-xs text-gray-400 mt-1">Sin trabajos asignados</div>
                    </div>
                </div>
            </div>

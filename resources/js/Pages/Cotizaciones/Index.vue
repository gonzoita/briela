    ...
    <AppLayout title="Cotizaciones">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex items-center justify-between mb-4">
                <h1 class="text-lg font-bold text-gray-900">Cotizaciones</h1>
                <div class="flex items-center gap-2">
                    <button @click="mostrarMetricas = !mostrarMetricas"
                        class="p-2 rounded-xl border border-gray-200 text-gray-400 hover:text-gray-700 hover:bg-gray-50 transition-colors"
            ...
            <div v-if="mostrarMetricas" class="mb-4">
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3">
                        <p class="text-xs text-gray-400 mb-1">Borradores</p>
                        <p class="text-2xl font-bold text-gray-800">{{ metricas?.borradores ?? 0 }}</p>
                    </div>
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3">
                        <p class="text-xs text-gray-400 mb-1">Enviadas</p>
                        <p class="text-2xl font-bold" style="color:#1D4ED8;">{{ metricas?.enviadas ?? 0 }}</p>
                    </div>
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3">
                        <p class="text-xs text-gray-400 mb-1">Aprobadas</p>
                        <p class="text-2xl font-bold" style="color:#065F46;">{{ metricas?.aprobadas ?? 0 }}</p>
                    </div>
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3">
                        <p class="text-xs text-gray-400 mb-1">Aprobado este mes</p>
                        <p class="text-xl font-bold" style="color:var(--marca);">${{ formatCOP(metricas?.total_mes ?? 0) }}</p>
                    </div>
                </div>
                ...
                <div v-if="metricas?.necesitan_seguimiento > 0"
                    ...
                    <div class="flex items-center gap-3 mb-2">
                        <svg class="w-5 h-5 shrink-0" style="color:#92400E;" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3">
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Aprobadas — últimos 6 meses</p>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4 mb-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    <input v-model="filters.buscar" type="text" placeholder="Buscar número o cliente..."
                        class="rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:border-blue-400"/>
                    <select v-model="filters.estado"
                        class="rounded-xl border border-gray-300 px-3 py-2 text-sm text-gray-700 focus:outline-none focus:border-blue-400">
                    ...
                    <div class="flex items-center gap-2">
                        <input v-model="filters.desde" type="date"
                            class="flex-1 min-w-0 rounded-xl border border-gray-300 px-3 py-2 text-sm text-gray-700 focus:outline-none focus:border-blue-400"/>
                ...
                <div v-if="filters.buscar || filters.estado || filters.responsable_id || filters.desde || filters.hasta"
                    ...
                    <button @click="limpiarFiltros"
                        class="text-xs text-gray-500 hover:text-red-600 flex items-center gap-1 transition-colors">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr style="background:#F8FAFC; border-bottom: 1px solid #E5E7EB;">
                                <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Número</th>
                                <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Cliente</th>
                                <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Responsable</th>
                                <th class="text-center px-3 py-3 text-xs font-semibold text-gray-500 uppercase">Ítems</th>
                                <th class="text-center px-3 py-3 text-xs font-semibold text-gray-500 uppercase">Creación</th>
                                <th class="text-center px-3 py-3 text-xs font-semibold text-gray-500 uppercase">Validez</th>
                                <th class="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Total</th>
                                <th class="text-center px-3 py-3 text-xs font-semibold text-gray-500 uppercase">Estado</th>
                                <th class="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Acciones</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-50">
                            <tr v-for="cot in cotizaciones.data" :key="cot.id" class="hover:bg-gray-50 transition-colors">
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3 text-gray-500 text-xs">{{ cot.responsable?.name }}</td>
                                <td class="px-3 py-3 text-center">
                                    <span class="text-xs text-gray-500 font-medium">{{ cot.items_count ?? 0 }}</span>
                                </td>
                                <td class="px-3 py-3 text-center text-gray-500 text-xs">{{ formatFecha(cot.fecha_creacion) }}</td>
                                <td class="px-3 py-3 text-center text-gray-500 text-xs">{{ formatFecha(cot.fecha_validez) }}</td>
                                <td class="px-4 py-3 text-right font-semibold text-gray-800">${{ formatCOP(cot.total) }}</td>
                                <td class="px-3 py-3 text-center">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-end gap-1">
                                        <a :href="`/cotizaciones/${cot.id}`" class="p-1.5 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50" title="Ver">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                            </svg>
                                        </a>
                                        <a :href="`/cotizaciones/${cot.id}/editar`" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-700 hover:bg-gray-100" title="Editar">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                            </svg>
                                        </a>
                                        <a :href="`/cotizaciones/${cot.id}/pdf`" target="_blank" class="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50" title="PDF">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/>
                                            </svg>
                                        </a>
                                        <button @click="duplicar(cot)" class="p-1.5 rounded-lg text-gray-400 hover:text-green-600 hover:bg-green-50" title="Duplicar">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div class="md:hidden divide-y divide-gray-100">
                    <div v-if="cotizaciones.data.length === 0" class="px-4 py-12 text-center text-gray-400 text-sm">
                        Sin cotizaciones.
                    </div>
                    <div v-for="cot in cotizaciones.data" :key="cot.id" class="p-4">
                        <div class="flex items-center justify-between mb-2">
                            <a :href="`/cotizaciones/${cot.id}`" class="font-bold text-sm" style="color:var(--marca);">{{ cot.numero }}</a>
                            <span class="text-xs px-2 py-0.5 rounded-full font-medium"
                                :style="`background:${cot.estado_badge?.bg};color:${cot.estado_badge?.text};`">
                                {{ cot.estado_badge?.label }}
                            </span>
                        </div>
                        <p class="text-sm text-gray-700 font-medium">{{ cot.cliente?.nombre ?? cot.nombre_contacto_override ?? '—' }}</p>
                        ...
                        <div class="flex items-center justify-between mt-2">
                            ...
                            <p class="text-sm font-bold" style="color:var(--marca);">${{ formatCOP(cot.total) }}</p>
                        </div>
                        <div class="flex gap-2 mt-3">
                            <a :href="`/cotizaciones/${cot.id}`" class="flex-1 text-center py-2 rounded-xl border border-gray-200 text-xs text-gray-600">Ver</a>
                            <a :href="`/cotizaciones/${cot.id}/editar`" class="flex-1 text-center py-2 rounded-xl border border-gray-200 text-xs text-gray-600">Editar</a>

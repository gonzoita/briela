    ...
    <AppLayout title="Nueva Solicitud de Compra">
        <div class="max-w-3xl mx-auto px-4 py-4">
            ...
            <div class="flex items-center gap-3 mb-4">
                <a href="/compras/solicitudes" class="text-gray-400 hover:text-gray-600">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4 mb-4 space-y-3">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">OP vinculada (opcional)</label>
                        <select v-model="form.op_id" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4 mb-4">
                <h2 class="font-semibold text-gray-900 mb-3">Ítems a solicitar</h2>
                ...
                <div class="relative mb-3">
                    ...
                    <div v-if="buscarItem && itemsFiltrados.length" class="absolute z-10 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                        <button v-for="item in itemsFiltrados.slice(0, 8)" :key="item.id"
                            ...
                            <span class="text-gray-400 ml-2 text-xs">{{ item.codigo }} · {{ item.unidad }}</span>
                        </button>
                    </div>
                </div>
                        ...
                        <div class="space-y-2">
                            <div>
                                ...
                                <input v-model="item.descripcion" type="text"
                                    class="w-full rounded border border-gray-300 px-2 py-1.5 text-sm focus:ring-1 focus:ring-blue-500 focus:outline-none" />
                            </div>
                            <div class="grid grid-cols-3 gap-2">
                                ...
                                <div>
                                    <label class="block text-xs text-gray-600 mb-0.5">Precio est.</label>
                                    <input v-model="item.precio_estimado" type="number" min="0" step="0.01"
                                        class="w-full rounded border border-gray-300 px-2 py-1.5 text-sm" />
                                </div>
                            </div>
                            <div>
                                <label class="block text-xs text-gray-600 mb-0.5">Notas</label>
            ...
            <div class="flex gap-3">
                <button @click="guardarBorrador" :disabled="guardando"
                    class="flex-1 py-3 rounded-xl border border-gray-300 text-sm font-medium text-gray-700 disabled:opacity-50">
                    Guardar borrador
                </button>

    ...
    <div class="min-h-screen flex flex-col" style="background:#F8F9FA;">
        ...
        <main class="flex-1 px-6 max-w-md mx-auto w-full">
            ...
            <div class="rounded-2xl shadow-sm p-6 bg-white mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Código del certificado</label>
                <input
                    v-model="codigo"
                    type="text"
            ...
            <div v-if="certificado?.valido"
                ...
                <div class="flex items-center gap-2 mb-4">
                    <svg class="w-7 h-7" style="color:#065F46;" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    <p class="text-lg font-bold" style="color:#065F46;">Certificado válido</p>
                </div>
                <dl class="space-y-2 text-sm">
                    <div class="flex justify-between gap-4">
                        <dt class="text-gray-500">Otorgado a</dt>
                        <dd class="font-semibold text-gray-900 text-right">{{ certificado.estudiante }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-gray-500">Curso</dt>
                        <dd class="font-semibold text-gray-900 text-right">{{ certificado.curso }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-gray-500">Fecha de emisión</dt>
                        <dd class="font-semibold text-gray-900 text-right">{{ certificado.fecha_emision }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-gray-500">Código</dt>
                        <dd class="font-mono text-gray-700 text-right">{{ certificado.codigo }}</dd>
                    </div>
                </dl>
            </div>
            ...
            <div v-else-if="noEncontrado"
                ...
                <svg class="w-7 h-7 mx-auto mb-2" style="color:#991B1B;" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
                </svg>
                <p class="text-base font-bold" style="color:#991B1B;">Certificado no encontrado</p>

...
const rolConfig = {
    ...
    operario:        { label: 'Operario',           class: 'bg-gray-100 text-gray-700' },
}
...
<template>
    <AppLayout title="Usuarios">
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        >
                            ...
                            <td class="px-6 py-4 text-gray-600">
                                <div>{{ u.email }}</div>
                                <div v-if="u.telefono" class="text-xs text-gray-400 mt-0.5">{{ u.telefono }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                                    :class="rolConfig[u.rol]?.class ?? 'bg-gray-100 text-gray-700'"
                                >
                                    {{ u.rol_nombre ?? rolConfig[u.rol]?.label ?? u.rol }}
                                </span>
                                <div v-if="u.sede" class="text-xs text-gray-400 mt-1">{{ u.sede }}</div>
                            </td>
            ...
            <!-- Paginación -->
            <div v-if="usuarios.last_page > 1" class="flex items-center justify-between px-6 py-4 border-t border-gray-100">

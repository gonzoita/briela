...
function onImagen(e) {
    ...
    previewImagen.value = file ? URL.createObjectURL(file) : null
}

// ── Imagen generada con IA ────────────────────────────────────────────────────
// La imagen se guarda en Multimedia y además se adjunta a esta publicación,
// para poder reutilizarla después sin volver a generarla (ni volver a pagarla).
...
async function generarImagen() {
    ...
    try {
        ...
        // Se descarga la imagen ya guardada y se adjunta al formulario, para
        // que siga el mismo camino que una imagen subida a mano.
...
function enviar(accion) {
    form.accion = accion
    form.post('/rrss', { forceFormData: true })
}
</script>

<template>
    <AppLayout title="Nueva publicación">
        <div class="max-w-2xl mx-auto px-4 py-4">
            ...
            <div class="flex items-center gap-3 mb-5">
                ...
                <h1 class="text-xl font-bold text-gray-900">Nueva publicación</h1>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
                ...
                <!-- Contenido -->
                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Texto *</label>
                    ...
                    <p class="text-xs text-gray-400 mt-1 text-right">{{ form.contenido.length }}/3000</p>
                ...
                <div>
                    <div class="flex items-center justify-between mb-1.5">
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider">Imagen</label>
                    ...
                    <!-- Generador con IA -->
                    <div v-if="iaAbierto" class="rounded-xl border border-gray-200 bg-gray-50 p-3 mb-2 space-y-2">
                        ...
                        <label class="flex items-center gap-2 cursor-pointer">
                            ...
                            <span class="text-xs text-gray-600">Mejorar mi descripción con IA</span>
                        </label>
                    ...
                    <input type="file" accept="image/*" @change="onImagen"
                        class="w-full text-sm text-gray-600 file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:bg-gray-100 file:text-sm" />
                ...
                <!-- Fecha programada -->
                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Fecha y hora *</label>
                ...
                <!-- Cuentas destino -->
                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Publicar en *</label>
                    <div v-for="(lista, red) in cuentasPorRed" :key="red" class="mb-2">
                        <p class="text-xs font-semibold text-gray-600 mb-1">{{ redIcono[red] }} {{ redLabel[red] ?? red }}</p>
                        <label v-for="c in lista" :key="c.id"
                            ...
                            <span class="text-sm text-gray-700">{{ c.nombre_cuenta }}</span>
                        </label>
                    </div>

    ...
    <AppLayout title="Colaboradores">
        <div class="max-w-5xl mx-auto">
            ...
            <div class="flex items-center justify-between mb-5 gap-2 flex-wrap">
                <h1 class="text-xl font-semibold text-tinta-900">Colaboradores</h1>
                <div class="flex items-center gap-2">
                    <button @click="modalBonos = true"
                        class="px-4 py-2 rounded-xl text-sm font-semibold border border-tinta-200 text-tinta-700 hover:bg-tinta-50">
                ...
                <div v-if="modalBonos" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4"
                    ...
                    <div class="bg-white rounded-2xl w-full max-w-sm p-5 space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="text-xs text-tinta-400 block mb-1">Mes</label>
                                <select v-model.number="bonoMes"
                                    class="w-full border border-tinta-200 rounded-lg px-3 py-2 text-sm">
                                    <option v-for="(m, i) in meses" :key="i" :value="i + 1">{{ m }}</option>
                                </select>
                            </div>
                            <div>
                                <label class="text-xs text-tinta-400 block mb-1">Año</label>
            ...
            <div class="bg-white rounded-xl border border-linea p-4 mb-4 space-y-3">
                ...
                <div class="flex flex-wrap gap-2 items-center">
                    ...
                    <input v-model="filters.puntos_min" type="number"
                        ...
                        class="w-36 rounded-lg border border-tinta-200 px-3 py-2 text-sm focus:outline-none" />

                    <button @click="limpiarFiltros"
                        class="px-3 py-2 rounded-lg text-sm text-tinta-400 hover:bg-tinta-100 transition-colors">
                        Limpiar
                    </button>
                </div>
            </div>
                ...
                <a v-for="op in operarios" :key="op.id"
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-2 flex-wrap">
                            <span class="font-semibold text-tinta-900 text-sm">{{ op.nombre }}</span>
                        ...
                        <div class="flex items-center gap-2 mt-1.5 flex-wrap">
                            ...
                            <span v-if="op.fecha_ingreso" class="text-xs text-tinta-300">
                    ...
                    <svg class="w-4 h-4 text-tinta-300 flex-shrink-0 mt-1"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            stroke-width="1.6" d="M9 5l7 7-7 7"/>
                    </svg>
                </a>
            </div>

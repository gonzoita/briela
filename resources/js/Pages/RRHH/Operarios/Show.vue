        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="flex items-center gap-3 mb-5">
                ...
                <div class="flex-1 min-w-0">
                    <h1 class="text-xl font-bold text-gray-900">{{ operario.nombre }}</h1>
                    <p class="text-xs text-gray-400">{{ operario.documento }} · {{ operario.especialidad ?? 'Sin especialidad' }}</p>
                </div>
                <span class="text-xs px-2.5 py-1 rounded-full font-semibold"
                    :class="operario.estado === 'activo' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'">
                    {{ operario.estado }}
                </span>
            ...
            <div v-show="tab === 'resumen'" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="bg-white rounded-2xl border border-gray-200 p-5">
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Información</p>
                        <div class="space-y-2 text-sm">
                            ...
                            <div class="flex justify-between">
                                <span class="text-gray-500">Usuario sistema</span>
                    ...
                    <div class="bg-white rounded-2xl border border-gray-200 p-5">
                        ...
                        <div v-if="bono_mes" class="space-y-1.5 text-sm">
                            ...
                            <div class="flex justify-between">
                                <span class="text-gray-500">Hitos</span>
                            ...
                            <div class="flex justify-between font-bold text-base border-t border-gray-100 pt-2 mt-2">
                                <span>Total</span>
                                <span class="text-green-700">${{ Number(bono_mes.total_bono).toLocaleString('es-CO') }}</span>
                            </div>
                        </div>
                        <div v-else class="text-sm text-gray-400 italic">Sin bono calculado.</div>
                ...
                <!-- Historial bonos -->
                <div v-if="historial_bonos.length" class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <div class="px-5 py-3 border-b border-gray-100">
                        <h2 class="text-sm font-semibold text-gray-700">Historial de Bonos</h2>
                    </div>
                    <div class="divide-y divide-gray-50">
                        <div v-for="b in historial_bonos" :key="b.id"
                            class="flex items-center justify-between px-5 py-3">
                            <span class="text-sm text-gray-700">{{ mesLabel(b.periodo_mes) }} {{ b.periodo_anio }}</span>
            ...
            <div v-show="tab === 'puntos'" class="space-y-4">
                ...
                <div class="flex items-center gap-3 p-4 rounded-2xl border-2"
                    ...
                    <div class="flex-1">
                        <p class="text-xs text-gray-500 uppercase tracking-wide">Nivel actual</p>
                        <p class="text-2xl font-bold" :style="{ color: nivelColor }">
                            {{ operario.nivel ?? 'Bronce' }}
                        </p>
                        <p class="text-sm text-gray-600">{{ operario.puntos_totales ?? 0 }} puntos acumulados</p>
                    </div>
                </div>

                <!-- Agregar puntos manual -->
                <div class="bg-white rounded-2xl border border-gray-200 p-5">
                    <h3 class="text-sm font-semibold text-gray-700 mb-4">Ajuste manual de puntos</h3>
                    <div class="space-y-3">
                        <div>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Concepto *</label>
                ...
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <div class="px-5 py-3 border-b border-gray-100">
                        <h2 class="text-sm font-semibold text-gray-700">Historial de puntos</h2>
                    </div>
                    <div v-if="!historial_puntos?.length" class="py-10 text-center text-sm text-gray-400">
                    ...
                    <div v-else class="divide-y divide-gray-50">
                        <div v-for="p in historial_puntos" :key="p.id"
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-400 mt-0.5">{{ p.created_at }}</p>
                            </div>
            ...
            <div v-show="tab === 'documentos'" class="space-y-4">
                <!-- Seguridad social -->
                <div class="bg-white rounded-2xl border border-gray-200 p-5">
                    <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Seguridad Social</p>
                    <div class="space-y-3 text-sm">
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                <p class="text-xs text-gray-400">EPS</p>
                                <p class="font-medium">{{ operario.nombre_eps || '—' }}</p>
                                <p class="text-xs text-gray-400">{{ operario.numero_eps || '' }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-400">Pensión</p>
                                <p class="font-medium">{{ operario.nombre_pension || '—' }}</p>
                                <p class="text-xs text-gray-400">{{ operario.numero_pension || '' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Info bancaria -->
                <div class="bg-white rounded-2xl border border-gray-200 p-5">
                    <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Información Bancaria</p>
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <div class="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
                        <h2 class="text-sm font-semibold text-gray-700">Archivos</h2>
                        <a :href="`/rrhh/operarios/${operario.id}/editar`"
                            @click.prevent="router.visit(`/rrhh/operarios/${operario.id}/editar`)"
                    ...
                    <div class="divide-y divide-gray-50">
                        ...
                        <div v-if="operario.archivo_otros?.length" class="px-5 py-3">
                            <p class="text-sm text-gray-700 mb-1">Otros documentos</p>
                ...
                <!-- Gamificación -->
                <div class="bg-white rounded-2xl border border-gray-200 p-5">
                    <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Gamificación</p>
                    <div class="flex items-center gap-4">
                        <div class="text-center">
                            <p class="text-2xl font-bold text-blue-600">{{ operario.puntos_totales ?? 0 }}</p>
                            <p class="text-xs text-gray-400">Puntos totales</p>
                        </div>
                        <div class="text-center">
                            <p class="text-lg font-bold text-gray-700">{{ operario.nivel || '—' }}</p>
            ...
            <div v-show="tab === 'trabajos'" class="space-y-4">
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    ...
                    <div v-if="!pasos_mes.length" class="py-10 text-center text-sm text-gray-400">
                    ...
                    <div v-else class="divide-y divide-gray-50">
                        <div v-for="paso in pasos_mes" :key="paso.id" class="px-5 py-3">
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-gray-800">{{ paso.nombre }}</p>
                                    <p class="text-xs text-gray-400 mt-0.5 truncate">
                                        OP: {{ paso.trabajo?.op_item?.op?.numero ?? '—' }}
                                    </p>
                                </div>
                                <div class="text-right shrink-0">
                                    <p class="text-xs text-gray-500">{{ formatFecha(paso.completado_at) }}</p>
            ...
            <div v-show="tab === 'disciplina'" class="space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <div class="px-5 py-3 border-b border-gray-100">
                        <h2 class="text-sm font-semibold text-gray-700">Registros de disciplina</h2>
                    </div>
                    <div v-if="!disciplinas.length" class="py-10 text-center text-sm text-gray-400">Sin registros.</div>
                    <div v-else class="divide-y divide-gray-50">
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2 mb-1">
                                        ...
                                        <span class="text-xs text-gray-500">{{ d.tipo_label }}</span>
                                    </div>
                                    <p class="text-sm text-gray-700">{{ d.descripcion }}</p>
                                    <p class="text-xs text-gray-400 mt-1">{{ formatFecha(d.fecha) }}</p>
                                </div>
                                <button v-if="!d.firmado"
                                    @click="firmarDisciplina(d)"
                                    class="shrink-0 px-3 py-1.5 rounded-xl border border-gray-300 text-xs font-medium text-gray-700 hover:bg-gray-50">
                                    Firmar
                                </button>
                            </div>
                        </div>
                ...
                <!-- Nuevo disciplina -->
                <div class="bg-white rounded-2xl border border-gray-200 p-5">
                    <h3 class="text-sm font-semibold text-gray-700 mb-4">Nuevo registro</h3>
                    <div class="space-y-3">
                        <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Penalización $</label>
            ...
            <div v-show="tab === 'bonos'" class="space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <div class="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
                        <h2 class="text-sm font-semibold text-gray-700">Hitos — {{ mesLabel(mes_actual) }}</h2>
                        <button @click="calcularBono"
                    ...
                    <div v-if="!hitos_mes.length" class="py-8 text-center text-sm text-gray-400">Sin hitos este mes.</div>
                    <div v-else class="divide-y divide-gray-50">
                        <div v-for="h in hitos_mes" :key="h.id" class="flex items-center px-5 py-3 gap-3">
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-medium text-gray-800">{{ h.nombre }}</p>
                                <p class="text-xs text-gray-400">Meta: {{ h.meta_valor }} {{ h.meta_tipo }}</p>
                            </div>
                            <span class="text-xs px-2 py-0.5 rounded-full font-semibold shrink-0"
                                :class="h.cumplido ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'">
                ...
                <!-- Agregar hito -->
                <div class="bg-white rounded-2xl border border-gray-200 p-5">
                    <h3 class="text-sm font-semibold text-gray-700 mb-4">Nuevo hito manual</h3>
                    <div class="space-y-3">
                        <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Meta tipo</label>
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Meta valor</label>
                                <input v-model.number="hitoForm.meta_valor" type="number" min="0"
                                    class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2" />
                            </div>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Valor bono ($)</label>
            ...
            <div v-show="tab === 'extras'" class="space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <div class="px-5 py-3 border-b border-gray-100">
                        <h2 class="text-sm font-semibold text-gray-700">Horas extras — {{ mesLabel(mes_actual) }}</h2>
                    </div>
                    <div v-if="!horas_extras_mes.length" class="py-8 text-center text-sm text-gray-400">Sin horas extras este mes.</div>
                    <div v-else class="divide-y divide-gray-50">
                        <div v-for="h in horas_extras_mes" :key="h.id" class="flex items-center px-5 py-3 gap-3">
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-medium text-gray-800 capitalize">{{ h.tipo }}</p>
                                <p class="text-xs text-gray-400">{{ formatFecha(h.fecha) }}</p>
                            </div>
                            <span class="text-sm font-bold text-gray-700 shrink-0">{{ h.horas }}h</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-5">
                    <h3 class="text-sm font-semibold text-gray-700 mb-4">Registrar horas extras</h3>
                    <div class="space-y-3">
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Fecha *</label>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Horas *</label>
                            <input v-model.number="extraForm.horas" type="number" min="0.5" max="24" step="0.5"
                                class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2" />
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Observación</label>
                ...
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <div class="px-5 py-3 border-b border-gray-100">
                        <h2 class="text-sm font-semibold text-gray-700">Permisos — {{ mesLabel(mes_actual) }}</h2>
                    </div>
                    <div v-if="!permisos_mes.length" class="py-6 text-center text-sm text-gray-400">Sin permisos este mes.</div>
                    <div v-else class="divide-y divide-gray-50">
                        <div v-for="p in permisos_mes" :key="p.id" class="flex items-center px-5 py-3 gap-3">
                            <div class="flex-1 min-w-0">
                                ...
                                <p v-if="p.motivo" class="text-xs text-gray-400 truncate mt-0.5">{{ p.motivo }}</p>
                            </div>
                ...
                <!-- Agregar permiso -->
                <div class="bg-white rounded-2xl border border-gray-200 p-5">
                    <h3 class="text-sm font-semibold text-gray-700 mb-4">Registrar permiso</h3>
                    <div class="space-y-3">
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Motivo</label>

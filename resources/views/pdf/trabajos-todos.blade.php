...
.header { background: {{ $marcaColor }}; color: white; padding: 14px 20px; }
...
.section { padding: 10px 20px; border-bottom: 1px solid #E5E7EB; }
.section-title { font-size: 8px; font-weight: bold; color: #6B7280; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px; }
...
.variable-box { border: 1px solid #FDE68A; background: #FFFBEB; padding: 8px; margin-bottom: 6px; border-radius: 4px; display: table; width: 100%; }
...
.variable-value { font-size: 14px; font-weight: bold; color: #78350F; }
...
table.componentes th { background: #F3F4F6; border: 1px solid #D1D5DB; padding: 5px 8px; font-size: 8px; font-weight: bold; text-align: left; text-transform: uppercase; color: #374151; }
table.componentes td { border: 1px solid #E5E7EB; padding: 5px 8px; font-size: 9px; }
table.componentes tr:nth-child(even) td { background: #F9FAFB; }

.paso-row { border: 1px solid #E5E7EB; padding: 8px 10px; margin-bottom: 5px; border-radius: 4px; display: table; width: 100%; }
...
.paso-nombre { font-size: 10px; font-weight: bold; color: #111; }
...
.qr-section { display: table; width: 100%; padding: 10px 20px; }
...
.firma-line { border-top: 1px solid #374151; margin-top: 30px; padding-top: 4px; font-size: 9px; color: #374151; }
...
@php
    ...
    if ($trabajo->token_trabajo) {
        ...
        $qrTrabajo  = 'data:image/svg+xml;base64,' . base64_encode(
            \SimpleSoftwareIO\QrCode\Facades\QrCode::format('svg')->size(120)->generate($urlTrabajo)
        );
    }
@endphp
...
{{-- COMPONENTES (solo en la primera unidad para ahorrar papel) --}}
@if($loop->first && $item->componentes->count())
<div class="section">

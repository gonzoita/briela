...
.hero-img img { width: 180px; height: 165px; object-fit: contain; border-radius: 8px; border: 1px solid #ddd6fe; background: #fff; }
.hero-img-placeholder { width: 180px; height: 165px; background: #ede9fe; border-radius: 8px; border: 1px solid #ddd6fe; display: flex; align-items: center; justify-content: center; font-size: 10px; color: #9ca3af; }
.hero-info { flex: 1; }
...
/* GRID INFO */
.info-grid { display: flex; gap: 0; border: 1px solid #e5e7eb; border-radius: 6px; overflow: hidden; margin: 14px 24px 0; }
  ...
  <div class="hero-inner">
    <div class="hero-img">
      @if($imagenPath && file_exists($imagenPath))
        <img src="{{ $imagenPath }}" alt="{{ $ensamble->nombre }}" />
      @else
    ...
    <div class="hero-info">
      <div class="hero-badges">
        @if($ensamble->plantilla)
        <span class="badge-plantilla">{{ $ensamble->plantilla->nombre }}</span>
        @endif
        @if($ensamble->categoria)
        <span class="badge-cat">{{ $ensamble->categoria->nombre }}</span>
        @endif
      </div>
      <div class="hero-nombre">{{ $ensamble->nombre }}</div>
      @if($ensamble->descripcion_corta)
      <div class="hero-desc">{{ $ensamble->descripcion_corta }}</div>
      @endif
    </div>
  </div>
</div>
...
<div class="info-grid">
  <div class="info-cell">
    ...
    <div class="info-value">{{ $ensamble->plantilla?->nombre ?? '—' }}</div>
  </div>
  <div class="info-cell">
    <div class="info-label">Categoría</div>
    <div class="info-value">{{ $ensamble->categoria?->nombre ?? '—' }}</div>
  </div>
  <div class="info-cell">
  ...
  <div class="seccion" style="margin-top:14px;">
    ...
    <div class="precios-grid">
      <div class="precio-card">
        ...
        <div class="precio-valor">${{ number_format($ensamble->precio_mayorista, 0, ',', '.') }}</div>
      </div>
      <div class="precio-card">
        <div class="precio-canal">Distribuidor</div>
        <div class="precio-valor">${{ number_format($ensamble->precio_distribuidor, 0, ',', '.') }}</div>
      </div>
      <div class="precio-card highlight">
        <div class="precio-canal">Cliente final</div>
        <div class="precio-valor">${{ number_format($ensamble->precio_cliente_final, 0, ',', '.') }}</div>
      </div>
    </div>
    <div class="precio-nota">* Precios base en COP · El precio final depende de la configuración · IVA no incluido · Sujetos a cambio sin aviso previo</div>
  </div>
  @endif

  <!-- DETALLES TÉCNICOS -->
  @if($ensamble->descripcion_larga)
  <div class="seccion" style="{{ ($mostrarPrecio ?? true) ? '' : 'margin-top:14px;' }}">
    <div class="seccion-titulo">Detalles técnicos</div>
    <div class="seccion-texto">{!! $ensamble->descripcion_larga !!}</div>
  </div>
  @endif
